...
if (consensus) console.log("Running against bitcoinconsensus...");
...
function randomInput() {
  ...
  if (rand(0, 5) === 0) input.sequence = rand(0, 0xffffffff);
...
function randomTX() {
  ...
  for (let i = 0; i < inputs; i++) tx.inputs.push(randomInput());

  for (let i = 0; i < outputs; i++) tx.outputs.push(randomOutput());

  if (rand(0, 5) === 0) tx.locktime = rand(0, 0xffffffff);
...
function randomInputScript(redeem) {
  ...
  if (redeem) script.pushData(redeem);
...
function isPushOnly(script) {
  if (script.isPushOnly()) return true;

  for (const op of script.code) {
    if (op.value === Script.opcodes.NOP) continue;

    if (op.value === Script.opcodes.NOP_1) continue;

    if (op.value > Script.opcodes.NOP_3) continue;
...
function fuzzSimple(flags) {
  ...
  for (;;) {
    if (++total % 1000 === 0) console.log("Fuzzed %d scripts.", total);

    if (total % 500 === 0) tx = randomTX();
    ...
    try {
      ...
      if (e.type === "ScriptError") continue;
      throw e;
    }
    ...
    if (!stack.getBool(-1)) continue;

    if (isPushOnly(output)) continue;
...
function fuzzVerify(flags) {
  ...
  for (;;) {
    ...
    try {
      Script.verify(input, witness, output, tx, 0, 0, flags);
    } catch (e) {
...
function fuzzLess(flags) {
  ...
  for (;;) {
    ...
    try {
      Script.verify(ctx.input, ctx.witness, ctx.output, tx, 0, 0, flags);
    } catch (e) {
      if (e.type === "ScriptError") {
...
function main() {
  const flags =
    process.argv.indexOf("--standard") !== -1 ? STANDARD : MANDATORY;
