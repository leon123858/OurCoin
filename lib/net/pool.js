...
class Pool extends EventEmitter {
  ...
  async _disconnect() {
    for (const item of this.invMap.values()) item.resolve();
  ...
  async discoverGateway() {
    ...
    if (!this.options.upnp) return false;
  ...
  async discoverSeeds(checkPeers) {
    if (!this.options.discover) return;

    if (this.hosts.dnsSeeds.length === 0) return;
  ...
  async discoverExternal() {
    ...
    if (this.hosts.local.size > 0) return;
  ...
  resolveHeaders(peer) {
    ...
    for (let node = this.headerNext; node; node = node.next) {
      ...
      if (items.length === common.MAX_INV) break;
    }
  ...
  resolveHeight(hash, height) {
    ...
    for (let peer = this.peers.head(); peer; peer = peer.next) {
      if (!peer.bestHash || !peer.bestHash.equals(hash)) continue;
  ...
  async getItem(peer, item) {
    ...
    if (this.options.selfish) return null;
    ...
    if (this.chain.options.spv) return null;

    if (this.chain.options.prune) return null;
  ...
  async handleGetAddr(peer, packet) {
    if (this.options.selfish) return;
    ...
    for (const addr of addrs) {
      if (!peer.addrFilter.added(addr.hostname, "ascii")) continue;
  ...
  async _handleInv(peer, packet) {
    ...
    if (blocks.length > 0) await this.handleBlockInv(peer, blocks);

    if (txs.length > 0) await this.handleTXInv(peer, txs);
  }
  ...
  async handleBlockInv(peer, hashes) {
    ...
    if (!this.chain.synced && !peer.loader) return;

    if (this.options.hasWitness() && !peer.hasWitness()) return;
  ...
  async handleTXInv(peer, hashes) {
    ...
    if (this.syncing && !this.chain.synced) return;
  ...
  async handleGetCFilters(peer, packet) {
    if (!this.options.bip157) return;

    const stopHeight = await this.chain.getHeight(packet.stopHash);
    // The stop hash doesn't exist
    if (!stopHeight) return;

    if (stopHeight - packet.startHeight >= common.MAX_CFILTERS) return;
  ...
  async handleGetCFHeaders(peer, packet) {
    ...
    const stopHeight = await this.chain.getHeight(packet.stopHash);
    // The stop hash doesn't exist
    if (stopHeight < 0) return;

    if (stopHeight - packet.startHeight >= common.MAX_CFHEADERS) return;
  ...
  async handleGetBlocks(peer, packet) {
    ...
    if (this.chain.options.spv) return;

    if (this.chain.options.prune) return;

    let hash = await this.chain.findLocator(packet.locator);

    if (hash) hash = await this.chain.getNextHash(hash);

    const blocks = [];

    while (hash) {
      if (packet.stop && hash.equals(packet.stop)) break;
  ...
  async handleGetHeaders(peer, packet) {
    ...
    let entry;
    if (hash) entry = await this.chain.getEntry(hash);
    ...
    while (entry) {
      ...
      if (packet.stop && entry.hash.equals(packet.stop)) break;
  ...
  fillOutbound() {
    ...
    for (let i = 0; i < need; i++) this.addOutbound();
  }
  ...
  removePeer(peer) {
    ...
    for (const hash of peer.blockMap.keys()) this.resolveBlock(peer, hash);

    for (const hash of peer.txMap.keys()) this.resolveTX(peer, hash);
  ...
  async hasBlock(hash) {
    ...
    // Check the chain.
    if (await this.chain.has(hash)) return true;
  ...
  hasTX(hash) {
    ...
    if (!this.mempool) {
      ...
      if (!this.txFilter.added(hash)) return true;
    } else {
      // Check the mempool.
  ...
  getServiceNames() {
    ...
    for (const [service, bit] of Object.entries(services)) {
      if (this.options.hasServices(bit)) enabled.push(service);
    }
  ...
  getFilterIndexer(filterType) {
    if (!this.filterIndexers) return null;
...
class PoolOptions {
  ...
  fromOptions(options) {
    ...
    assert(
      options.chain && typeof options.chain === "object",
    ...
    if (this.bip157) this.services |= common.services.NODE_COMPACT_FILTERS;
  ...
  _createSocket(port, host) {
    if (this.proxy) return socks.connect(this.proxy, port, host);
  ...
  _resolve(name) {
    if (this.onion) return socks.resolve(this.proxy, name);
...
class BroadcastItem extends EventEmitter {
  ...
  handleAck(peer) {
    setTimeout(() => {
      ...
      for (const job of this.jobs) job.resolve(true);
...
class NonceList {
  ...
  alloc(hostname) {
    for (;;) {
      ...
      if (this.map.has(nonce)) continue;
