...
class VersionPacket extends Packet {
  ...
  fromOptions(options) {
    ...
    if (options.services != null) this.services = options.services;

    if (options.time != null) this.time = options.time;

    if (options.remote) this.remote.fromOptions(options.remote);

    if (options.local) this.local.fromOptions(options.local);
    ...
    if (options.noRelay != null) this.noRelay = options.noRelay;
  ...
  fromReader(br) {
    ...
    if (br.left() > 0) this.agent = br.readVarString("ascii", 256);

    if (br.left() > 0) this.height = br.readI32();

    if (br.left() > 0) this.noRelay = br.readU8() === 0;
...
class PingPacket extends Packet {
  ...
  toWriter(bw) {
    if (this.nonce) bw.writeBytes(this.nonce);
    return bw;
  }
  ...
  fromReader(br) {
    if (br.left() >= 8) this.nonce = br.readBytes(8);
    return this;
  }
...
class AddrPacket extends Packet {
  ...
  toWriter(bw) {
    ...
    for (const item of this.items) item.toWriter(bw, true);
...
class InvPacket extends Packet {
  ...
  toWriter(bw) {
    ...
    for (const item of this.items) item.toWriter(bw);
  ...
  fromReader(br) {
    ...
    for (let i = 0; i < count; i++) this.items.push(InvItem.fromReader(br));
...
class GetCFiltersPacket extends Packet {
  ...
  constructor(filterType, startHeight, stopHash) {
    ...
    this.type = exports.types.GETCFILTERS;
...
class CFilterPacket extends Packet {
  ...
  constructor(filterType, blockHash, filterBytes) {
    ...
    this.cmd = "cfilter";
    this.type = exports.types.CFILTER;
...
class GetCFHeadersPacket extends Packet {
  ...
  constructor(filterType, startHeight, stopHash) {
    ...
    this.type = exports.types.GETCFHEADERS;
...
class CFHeadersPacket extends Packet {
  ...
  constructor(filterType, stopHash, previousFilterHeader, filterHashes) {
    ...
    this.cmd = "cfheaders";
    this.type = exports.types.CFHEADERS;
  ...
  toWriter(bw) {
    ...
    for (const hash of this.filterHashes) bw.writeHash(hash);
  ...
  fromReader(br) {
    ...
    assert(
      filterHashesLength <= common.MAX_CFHEADERS,
...
class GetCFCheckptPacket extends Packet {
  ...
  constructor(filterType, stopHash) {
    ...
    this.type = exports.types.GETCFCHECKPT;
...
class CFCheckptPacket extends Packet {
  ...
  constructor(filterType, stopHash, filterHeaders) {
    ...
    this.cmd = "cfcheckpt";
    this.type = exports.types.CFCHECKPT;
  ...
  toWriter(bw) {
    ...
    for (const hash of this.filterHeaders) bw.writeHash(hash);
  ...
  fromReader(br) {
    ...
    for (let i = 0; i < length; i++) this.filterHeaders.push(br.readHash());
...
class GetBlocksPacket extends Packet {
  ...
  toWriter(bw) {
    ...
    for (const hash of this.locator) bw.writeHash(hash);
  ...
  fromReader(br) {
    ...
    for (let i = 0; i < count; i++) this.locator.push(br.readHash());

    this.stop = br.readHash();

    if (this.stop.equals(consensus.ZERO_HASH)) this.stop = null;
...
class HeadersPacket extends Packet {
  ...
  getSize() {
    ...
    for (const item of this.items) size += item.getSize();
  ...
  fromReader(br) {
    ...
    for (let i = 0; i < count; i++) this.items.push(Headers.fromReader(br));
...
class BlockPacket extends Packet {
  ...
  getSize() {
    if (this.witness) return this.block.getSize();
  ...
  toWriter(bw) {
    if (this.witness) return this.block.toWriter(bw);
  ...
  toRaw() {
    if (this.witness) return this.block.toRaw();
...
class TXPacket extends Packet {
  ...
  getSize() {
    if (this.witness) return this.tx.getSize();
  ...
  toWriter(bw) {
    if (this.witness) return this.tx.toWriter(bw);
  ...
  toRaw() {
    if (this.witness) return this.tx.toRaw();
...
class RejectPacket extends Packet {
  ...
  getCode() {
    ...
    if (!code) return this.code.toString(10);
  ...
  toWriter(bw) {
    ...
    if (this.hash) bw.writeHash(this.hash);
  ...
  fromReason(code, reason, msg, hash) {
    if (typeof code === "string") code = RejectPacket.codes[code.toUpperCase()];

    if (!code) code = RejectPacket.codes.INVALID;

    if (code >= RejectPacket.codes.INTERNAL) code = RejectPacket.codes.INVALID;
...
class CmpctBlockPacket extends Packet {
  ...
  getSize() {
    if (this.witness) return this.block.getSize(true);
...
class BlockTxnPacket extends Packet {
  ...
  getSize() {
    if (this.witness) return this.response.getSize(true);
  ...
  toWriter(bw) {
    if (this.witness) return this.response.toWriter(bw);
  ...
  toRaw() {
    if (this.witness) return this.response.toRaw();
...
exports.fromRaw = function fromRaw(cmd, data) {
  switch (cmd) {
    ...
    case "getcfilters":
      return GetCFiltersPacket.fromRaw(data);
    case "cfilter":
      return CFilterPacket.fromRaw(data);
    case "getcfheaders":
      return GetCFHeadersPacket.fromRaw(data);
    case "cfheaders":
      return CFHeadersPacket.fromRaw(data);
    case "getcfcheckpt":
      return GetCFCheckptPacket.fromRaw(data);
    case "cfcheckpt":
      return CFCheckptPacket.fromRaw(data);
    default:
...
exports.GetCFiltersPacket = GetCFiltersPacket;
exports.CFilterPacket = CFilterPacket;
exports.GetCFHeadersPacket = GetCFHeadersPacket;
exports.CFHeadersPacket = CFHeadersPacket;
exports.GetCFCheckptPacket = GetCFCheckptPacket;
exports.CFCheckptPacket = CFCheckptPacket;
