...
class CompactBlock extends AbstractBlock {
  ...
  getSize(witness) {
    ...
    for (const [index, tx] of this.ptx) {
      ...
      if (witness) size += tx.getSize();
      else size += tx.getBaseSize();
    }
  ...
  writeRaw(bw, witness) {
    ...
    for (const [index, tx] of this.ptx) {
      ...
      if (witness) tx.toWriter(bw);
      else tx.toNormalWriter(bw);
    }
  ...
  init() {
    ...
    for (let i = 0; i < this.totalTX; i++) this.available.push(null);
    ...
    for (let i = 0; i < this.ids.length; i++) {
      ...
      while (this.available[i + offset]) offset += 1;
...
class TXResponse {
  ...
  fromReader(br) {
    ...
    for (let i = 0; i < count; i++) this.txs.push(TX.fromReader(br));
