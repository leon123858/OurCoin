...
class Peer extends EventEmitter {
  ...
  async initVersion() {
    ...
    if (this.destroyed) throw new Error("Peer was destroyed during handshake.");
  ...
  flushInv() {
    ...
    for (const item of queue) {
      if (!this.invFilter.added(item.hash)) continue;
  ...
  sendInv(items) {
    ...
    for (const item of items) this.invFilter.add(item.hash);
  ...
  destroy() {
    ...
    for (const job of jobs) job.reject(new Error("Peer was destroyed."));
  ...
  drain() {
    if (this.destroyed) return Promise.reject(new Error("Peer is destroyed."));

    if (this.drainSize === 0) return Promise.resolve();
  ...
  getItems(type, hashes) {
    ...
    for (const hash of hashes) items.push(new InvItem(type, hash));
  ...
  async handlePacket(packet) {
    if (this.destroyed) throw new Error("Destroyed peer sent a packet.");
    ...
    if (this.onPacket) await this.onPacket(packet);
  ...
  async handleVersion(packet) {
    if (this.version !== -1) throw new Error("Peer sent a duplicate version.");
  ...
  sendGetHeaders(locator, stop) {
    ...
    let hash = null;
    if (packet.locator.length > 0) hash = packet.locator[0];
  ...
  sendCFilter(filterType, blockHash, filter) {
    ...
    this.logger.debug(
      "Sending cfilter (type=%d, blockHash=%h).",
      filterType,
      blockHash
    );
  ...
  sendCFHeaders(filterType, stopHash, previousFilterHeader, filterHashes) {
    const packet = new packets.CFHeadersPacket(
      filterType,
      stopHash,
    ...
    this.logger.debug(
      "Sending cfheaders (type=%d, stop=%h, prev=%h).",
      filterType,
      stopHash,
      previousFilterHeader
    );
  ...
  sendCFCheckpt(filterType, stopHash, filterHeaders) {
    const packet = new packets.CFCheckptPacket(
      filterType,
      stopHash,
      filterHeaders
    );
    ...
    this.logger.debug(
      "Sending cfcheckpt (type=%d, stop=%h, len=%d).",
      filterType,
      stopHash,
  ...
  sendReject(code, reason, msg, hash) {
    ...
    if (msg) {
      this.logger.debug(
        "Rejecting %s %h (%s): code=%s reason=%s.",
        msg,
        hash,
        this.hostname(),
        code,
      ...
      this.logger.debug(
        "Rejecting packet from %s: code=%s reason=%s.",
        this.hostname(),
        code,
        reason
      );
  ...
  getServiceNames() {
    ...
    for (const [service, bit] of Object.entries(services)) {
      if (this.hasServices(bit)) enabled.push(service);
    }
...
class RequestEntry {
  ...
  resolve(result) {
    for (const job of this.jobs) job.resolve(result);
