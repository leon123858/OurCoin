...
class HostList {
  ...
  init() {
    ...
    for (let i = 0; i < options.maxBuckets; i++) this.fresh.push(new Map());

    for (let i = 0; i < options.maxBuckets; i++) this.used.push(new List());
    ...
    for (const host of hosts) this.addLocal(host, port, scores.IF);
  }
  ...
  reset() {
    ...
    for (const bucket of this.fresh) bucket.clear();

    for (const bucket of this.used) bucket.reset();
  ...
  getHost() {
    ...
    if (this.totalUsed > 0) {
      if (this.totalFresh === 0 || random(2) === 0) buckets = this.used;
    }
  ...
  add(addr, src) {
    ...
    if (entry) {
      ...
      let factor = 1;
      for (let i = 0; i < entry.refCount; i++) factor *= 2;

      if (random(factor) !== 0) return false;
    } else {
  ...
  remove(hostname) {
    ...
    if (entry.used) {
      ...
      for (const bucket of this.fresh) {
        if (bucket.delete(entry.key())) entry.refCount -= 1;
      }
  ...
  toArray() {
    ...
    for (const entry of this.map.values()) out.push(entry.addr);
  ...
  setSeeds(seeds) {
    ...
    for (const host of seeds) this.addSeed(host);
  }
  ...
  setNodes(nodes) {
    ...
    for (const host of nodes) this.addNode(host);
  }
  ...
  async discoverSeeds() {
    ...
    for (const seed of this.dnsSeeds) jobs.push(this.populateSeed(seed));
  ...
  async discoverNodes() {
    ...
    for (const node of this.dnsNodes) jobs.push(this.populateNode(node));
  ...
  toJSON() {
    ...
    for (const entry of this.map.values()) addrs.push(entry.toJSON());

    for (const bucket of this.fresh) {
      const keys = [];
      for (const key of bucket.keys()) keys.push(key);
      fresh.push(keys);
    }
  ...
  fromJSON(json) {
    ...
    assert(
      !json.network || json.network === this.network.type,
      "Network mistmatch."
    );

    assert(
      json.version === HostList.VERSION,
    ...
    for (const keys of json.fresh) {
      ...
      for (const key of keys) {
        ...
        assert(entry);
        if (entry.refCount === 0) totalFresh += 1;
        entry.refCount += 1;
    ...
    for (const entry of map.values()) assert(entry.used || entry.refCount > 0);
