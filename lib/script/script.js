...
class Script {
   ...
   * @param {String?} testTxid - Hex to test txid when no transaction.
   * @throws {ScriptError} Will be thrown on VERIFY failures.
   */

  async execute(stack, flags, tx, index, value, version, testTxid) {
    ...
    const txid = tx ? tx.txid() : testTxid || "testTxid";
    const state = [];
    const alt = [];
    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_DEPLOYCONTRACT: {
          ...
          const vm = new sandbox.sandbox(txid);
          await vm
            .initSandbox()
        ...
        case opcodes.OP_CALLCONTRACT: {
          if (stack.length < 2)
            throw new ScriptError("INVALID_STACK_OPERATION", op, ip);
          const contractAddr = stack.pop().toString("hex");
          const args = JSON.parse(stack.pop().toString("utf-8"));
          const vm = new sandbox.sandbox(contractAddr);
          await vm
            .initSandbox()
