...
const { ContractPool } = require("../contract/contractpool");
...
class Script {
  ...
  /**
   * check is script have contract operation
   * @returns {boolean}
   */

  isHaveContract() {
    return this.code.slice().some((opcode) => {
      return (
        opcode.value == opcodes.OP_CALLCONTRACT ||
        opcode.value == opcodes.OP_DEPLOYCONTRACT ||
        opcode.value == opcodes.OP_TRANSFERCONTRACT
      );
    });
  }
  ...
  async execute(stack, flags, tx, index, value, version) {
    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      if (
        op.value === opcodes.OP_CODESEPARATOR &&
        version === 0 &&
      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKLOCKTIMEVERIFY: {
          ...
          if (!(flags & Script.flags.VERIFY_CHECKLOCKTIMEVERIFY)) break;
        ...
        case opcodes.OP_CHECKSEQUENCEVERIFY: {
          ...
          if (!(flags & Script.flags.VERIFY_CHECKSEQUENCEVERIFY)) break;
        ...
        case opcodes.OP_ELSE: {
          ...
          if (!state[state.length - 1]) negate += 1;
          else negate -= 1;
        ...
        case opcodes.OP_DEPLOYCONTRACT: {
          if (stack.length < 3)
            throw new ScriptError("INVALID_STACK_OPERATION", op, ip);
          const code = stack.pop().toString("utf-8");
          const uid = stack.pop().toString("utf-8");
          const address = stack.pop().toString("utf-8");
          const vm = new Sandbox(uid);
          const pool = new ContractPool();
          const timeStamp = new Date().getTime();
          await pool
            .init()
            .then((pkg) => pool.deploy(timeStamp, uid, address))
            .catch((err) => {
              console.log(err);
              throw new ScriptError("CONTRACT_FAIL", op, ip);
            })
            .finally((pkg) => pool.close());

          await vm
            .initSandbox()
            .then((pkg) => vm.deploy(code))
            .catch((err) => {
              console.log(err);
              throw new ScriptError("CONTRACT_FAIL", op, ip);
            })
            .finally((pkg) => vm.closeSandbox());
          break;
        }
        case opcodes.OP_CALLCONTRACT: {
          if (stack.length < 2)
            throw new ScriptError("INVALID_STACK_OPERATION", op, ip);
          const uid = stack.pop().toString("utf-8");
          const args = JSON.parse(stack.pop().toString("utf-8"));
          const vm = new Sandbox(uid);
          const pool = new ContractPool();
          const timeStamp = new Date().getTime();
          ...
          await pool
            .init()
            .then((pkg) => pool.call(timeStamp, uid))
            .then((isLastest) => {
              if (!isLastest) {
                console.log("contract delay call");
                throw new ScriptError("CONTRACT_DELAY_CALL", op, ip);
              }
            })
            .catch((err) => {
              console.log(err);
        ...
        case opcodes.OP_TRANSFERCONTRACT: {
          if (stack.length < 2)
            throw new ScriptError("INVALID_STACK_OPERATION", op, ip);
          const uid = stack.pop().toString("utf-8");
          const newOwnerAddr = stack.pop().toString("utf-8");
          const pool = new ContractPool();
          const timeStamp = new Date().getTime();

          await pool
            .init()
            .then((pkg) => pool.transfer(timeStamp, newOwnerAddr, uid))
            .then((isLastest) => {
              if (!isLastest) {
                console.log("contract delay transfer");
                throw new ScriptError("CONTRACT_DELAY_TRANSFER", op, ip);
              }
            })
            .catch((err) => {
              console.log(err);
  ...
  getType() {
    ...
    if (this.isHaveContract()) return scriptTypes.CONTRACT;
  ...
  isPubkey(minimal) {
    ...
    return (
      (size === 33 || size === 65) && this.getOp(1) === opcodes.OP_CHECKSIG
    );
  }
      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKMULTISIGVERIFY:
          if (accurate && lastOp >= opcodes.OP_1 && lastOp <= opcodes.OP_16)
            total += lastOp - 0x50;
          else total += consensus.MAX_MULTISIG_PUBKEYS;
          break;
      }
  ...
  static async verify(input, witness, output, tx, index, value, flags) {
    if (flags == null) flags = Script.flags.STANDARD_VERIFY_FLAGS;
    ...
    await input.execute(stack, flags, tx, index, value, 0);
    ...
    await output.execute(stack, flags, tx, index, value, 0);
    ...
    if (flags & Script.flags.VERIFY_WITNESS && output.isProgram()) {
      ...
      await Script.verifyProgram(witness, output, flags, tx, index, value);
    ...
    if (flags & Script.flags.VERIFY_P2SH && output.isScripthash()) {
      ...
      await redeem.execute(stack, flags, tx, index, value, 0);
      ...
      if (flags & Script.flags.VERIFY_WITNESS && redeem.isProgram()) {
        ...
        await Script.verifyProgram(witness, redeem, flags, tx, index, value);
  ...
  static async verifyProgram(witness, output, flags, tx, index, value) {
    ...
    await redeem.execute(stack, flags, tx, index, value, 1);

    // Verify the stack values.
    if (stack.length !== 1) throw new ScriptError("CLEANSTACK");

    if (!stack.getBool(-1)) throw new ScriptError("EVAL_FALSE");
  }
...
function validateSignature(sig, flags) {
  ...
  if (
    flags & Script.flags.VERIFY_DERSIG ||
