...
class CPUMiner extends EventEmitter {
  ...
  init() {
    this.chain.on("tip", (tip) => {
      ...
      if (this.job.attempt.prevBlock.equals(tip.prevBlock)) this.job.destroy();
    });
  }
  ...
  async findNonceAsync(job) {
    if (!this.workers) return this.findNonce(job);
