...
class Miner extends EventEmitter {
  ...
  async _createBlock(tip, address) {
    ...
    if (version === -1) version = await this.chain.computeBlockVersion(tip);
  ...
  assemble(attempt) {
    ...
    assert(
      this.mempool.tip.equals(this.chain.tip.hash),
    ...
    for (const entry of this.mempool.map.values()) {
      ...
      for (const { prevout } of tx.inputs) {
        ...
        if (!this.mempool.hasEntry(hash)) continue;

        item.depCount += 1;

        if (!depMap.has(hash)) depMap.set(hash, []);
    ...
    while (queue.size() > 0) {
      ...
      if (priority) {
        if (
          weight > this.options.priorityWeight ||
    ...
    assert(
      attempt.weight <= consensus.MAX_BLOCK_WEIGHT,
    ...
    if (this.options.preverify) {
      ...
      assert(
        block.getWeight() <= attempt.weight,
        "Block exceeds reserved weight!"
      );

      assert(
        block.getBaseSize() <= consensus.MAX_BLOCK_SIZE,
...
class MinerOptions {
  ...
  fromOptions(options) {
    ...
    if (options.maxWeight != null) {
      ...
      assert(
        options.maxWeight <= consensus.MAX_BLOCK_WEIGHT,
    ...
    if (options.maxSigops != null) {
      ...
      assert(
        options.maxSigops <= consensus.MAX_BLOCK_SIGOPS_COST,
