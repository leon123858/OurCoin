...
class BlockTemplate {
  ...
  getWitnessHash() {
    ...
    for (const { tx } of this.items) leaves.push(tx.witnessHash());
  ...
  commit(proof) {
    ...
    for (const item of this.items) block.txs.push(item.tx);
...
class MerkleTree {
  ...
  withFirst(hash) {
    for (const step of this.steps) hash = hash256.root(hash, step);
    return hash;
  }
  ...
  toJSON() {
    ...
    for (const step of this.steps) steps.push(step.toString("hex"));
  ...
  fromItems(items) {
    ...
    for (const item of items) leaves.push(item.tx.hash());
