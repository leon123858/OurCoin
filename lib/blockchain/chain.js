...
class Chain extends AsyncEmitter {
  ...
  async verifyContext(block, prev, flags) {
    ...
    // duplicate txids.
    if (!state.hasBIP34()) await this.verifyDuplicates(block, prev);
  ...
  async getMedianTime(prev, time) {
    ...
    for (let i = 0; i < timespan && entry; i++) {
      ...
      if (cache) entry = cache;
      else entry = await this.getPrevious(entry);
    }
  ...
  async verify(block, prev, flags) {
    ...
    if (this.isHistorical(prev)) {
      if (this.options.spv) return this.state;
    ...
    if (flags & common.flags.VERIFY_BODY) {
      ...
      if (!valid) throw new VerifyError(block, "invalid", reason, score, true);
    }
  ...
  async getDeployments(time, prev) {
    ...
    if (this.options.bip148 && this.network === Network.main) {
      if (
        witness !== thresholdStates.LOCKED_IN &&
  ...
  async verifyDuplicates(block, prev) {
    for (const tx of block.txs) {
      if (!(await this.hasCoins(tx))) continue;
  ...
  async findFork(fork, longer) {
    while (!fork.hash.equals(longer.hash)) {
      while (longer.height > fork.height) {
        ...
        if (!longer) throw new Error("No previous entry for new tip.");
      }

      if (fork.hash.equals(longer.hash)) return fork;

      fork = await this.getPrevious(fork);

      if (!fork) throw new Error("No previous entry for old tip.");
    }
  ...
  async disconnect(entry) {
    ...
    if (!block) {
      if (!this.options.spv) throw new Error("Block not found.");
  ...
  async reconnect(entry) {
    ...
    try {
      ...
      if (err.type === "VerifyError") {
        if (!err.malleated) this.setInvalid(entry.hash);
        this.logger.warning(
  ...
  async setBestChain(entry, block, prev, flags) {
    ...
    if (!entry.prevBlock.equals(this.tip.hash)) {
      ...
      if (this.options.spv) return this.reorganizeSPV(entry);
  ...
  async saveAlternate(entry, block, prev, flags) {
    ...
    this.logger.warning(
      "Heads up: Competing chain at height %d:" +
  ...
  async _reset(block, silent) {
    ...
    if (!silent) await this.emitAsync("reset", tip);
  ...
  /**
   * execute contract in an output
   * @param {*} output script of output in transaction
   * @param {*} tx transaction hold output
   */
  ...
  /**
   * block want to execute contract in it
   * @param {*} block block have been add into chain
   */
  ...
  async add(block, flags, id) {
    ...
    try {
      const entry = await this._add(block, flags, id);
  ...
  async _add(block, flags, id) {
    ...
    // Handle any orphans.
    if (this.hasNextOrphan(hash)) await this.handleOrphans(entry);
  ...
  getProofTime(to, from) {
    ...
    if (work.bitLength() > 53) return sign * Number.MAX_SAFE_INTEGER;
  ...
  async getTarget(time, prev) {
    ...
    if ((prev.height + 1) % pow.retargetInterval !== 0) {
      if (pow.targetReset) {
        ...
        if (time > prev.time + pow.targetSpacing * 2) return pow.bits;
        ...
        while (
          ...
          if (cache) prev = cache;
          else prev = await this.getPrevious(prev);
  ...
  async findLocator(locator) {
    for (const hash of locator) {
      if (await this.isMainHash(hash)) return hash;
    }
  ...
  async getState(prev, deployment) {
    ...
    if (deployment.startTime === -1) return thresholdStates.ACTIVE;
    ...
    if ((prev.height + 1) % window !== 0) {
      ...
      if (!prev) return thresholdStates.DEFINED;
  ...
  async getLocks(prev, tx, view, flags) {
    ...
    if (!(flags & common.lockFlags.VERIFY_SEQUENCE)) return [-1, -1];

    if (tx.isCoinbase() || tx.version < 2) return [-1, -1];

    let minHeight = -1;
    let minTime = -1;

    for (const { prevout, sequence } of tx.inputs) {
      if (sequence & DISABLE_FLAG) continue;
  ...
  async verifyLocks(prev, tx, view, flags) {
    ...
    if (height !== -1) {
      if (height >= prev.height + 1) return false;
    }

    if (time !== -1) {
      const mtp = await this.getMedianTime(prev);

      if (time >= mtp) return false;
    }
...
class ChainOptions {
  ...
  fromOptions(options) {
    if (!options.spv) {
      assert(
        options.blocks && typeof options.blocks === "object",
