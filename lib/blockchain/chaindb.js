...
class ChainDB {
  ...
  async getHeight(hash) {
    ...
    if (hash.equals(consensus.ZERO_HASH)) return -1;
  ...
  async getEntryByHash(hash) {
    ...
    if (hash.equals(consensus.ZERO_HASH)) return null;
  ...
  async getAncestor(entry, height) {
    ...
    if (await this.isMainChain(entry)) return this.getEntryByHeight(height);
  ...
  async getNextEntry(entry) {
    ...
    // Not on main chain.
    if (!next.prevBlock.equals(entry.hash)) return null;
  ...
  async verifyFlags(state) {
    ...
    if (!flags) throw new Error("No flags found.");
    ...
    if (options.prune && !flags.prune) {
      if (!options.forceFlags) throw new Error("Cannot retroactively prune.");
      needsPrune = true;
    }
  ...
  async verifyDeployments() {
    ...
    try {
      ...
      if (e.type !== "EncodingError") throw e;
      invalid = [];
      for (let i = 0; i < 32; i++) invalid.push(i);
    }
  ...
  async invalidateCache(bit, b) {
    ...
    for (const key of keys) b.del(key);
  }
  ...
  async prune() {
    ...
    if (flags.prune) throw new Error("Chain is already pruned.");

    const height = await this.getHeight(this.state.tip);

    if (height <= pruneAfter + keepBlocks) return false;
    ...
    for (let i = start; i <= end; i++) {
      ...
      if (!hash) throw new Error(`Cannot find hash for ${i}.`);
  ...
  async isMainHash(hash) {
    ...
    if (hash.equals(consensus.ZERO_HASH)) return false;

    if (hash.equals(this.network.genesis.hash)) return true;

    if (hash.equals(this.state.tip)) return true;

    const cacheHash = this.cacheHash.get(hash);

    if (cacheHash) {
      const cacheHeight = this.cacheHeight.get(cacheHash.height);
      if (cacheHeight) return cacheHeight.hash.equals(hash);
    }

    if (await this.getNextHash(hash)) return true;
  ...
  async isMainChain(entry) {
    if (entry.isGenesis()) return true;

    if (entry.hash.equals(this.state.tip)) return true;

    const cache = this.getCache(entry.height);

    if (cache) return entry.hash.equals(cache.hash);

    if (await this.getNextHash(entry.hash)) return true;
  ...
  async readCoin(prevout) {
    if (this.options.spv) return null;
  ...
  async hasCoins(tx) {
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      if (await this.db.has(key)) return true;
    }
    return false;
  }
  ...
  async scan(start, filter, iter) {
    ...
    else this.logger.info("Scanning from block %h.", start);
  ...
  async _save(entry, block, view) {
    ...
    if (!entry.isGenesis()) this.put(layout.n.encode(entry.prevBlock), hash);
  ...
  async reset(block) {
    ...
    if (this.options.prune) throw new Error("Cannot reset when pruned.");
  ...
  async removeChains() {
    ...
    try {
      for (const tip of tips) await this._removeChain(tip);
    } catch (e) {
      this.drop();
      throw e;
    }
  ...
  async _removeChain(hash) {
    ...
    if (!tip) throw new Error("Alternate chain tip not found.");

    this.logger.debug("Removing alternate chain: %h.", tip.hash);

    for (;;) {
      if (await this.isMainChain(tip)) break;
  ...
  async removeBlock(entry) {
    if (this.options.spv) return new CoinView();
  ...
  async connectBlock(entry, block, view) {
    if (this.options.spv) return undefined;
    ...
    if (entry.isGenesis()) return undefined;
  ...
  async pruneBlock(entry) {
    ...
    if (height <= this.network.block.pruneAfterHeight) return;
...
class StateCache {
  ...
  init() {
    for (let i = 0; i < 32; i++) this.bits.push(null);
