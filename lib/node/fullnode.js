...
class FullNode extends Node {
  ...
  init() {
    ...
    if (this.txindex) this.txindex.on("error", (err) => this.error(err));

    if (this.addrindex) this.addrindex.on("error", (err) => this.error(err));
  ...
  async open() {
    ...
    if (this.txindex) await this.txindex.open();

    if (this.addrindex) await this.addrindex.open();
  ...
  async close() {
    ...
    if (this.txindex) await this.txindex.close();

    if (this.addrindex) await this.addrindex.close();
  ...
  async sendTX(tx) {
    ...
    if (this.pool.options.selfish) this.broadcast(tx);
  }
  ...
  startSync() {
    if (this.txindex) this.txindex.sync();

    if (this.addrindex) this.addrindex.sync();
  ...
  async getCoin(hash, index) {
    ...
    if (this.mempool.isSpent(hash, index)) return null;
  ...
  async getMetaByAddress(addr, options = {}) {
    if (!this.txindex || !this.addrindex) return [];
    ...
    if (reverse) unconfirmed();
    else await confirmed();

    if (metas.length > 0) limit -= metas.length;

    if (limit <= 0) return metas;

    if (reverse) await confirmed();
    else unconfirmed();
  ...
  async getMetaView(meta) {
    if (meta.height === -1) return this.mempool.getSpentView(meta.tx);

    if (this.txindex) return this.txindex.getSpentView(meta.tx);
  ...
  async getTXByAddress(addr, options = {}) {
    ...
    for (const mtx of mtxs) out.push(mtx.tx);
  ...
  async hasTX(hash) {
    if (this.mempool.hasEntry(hash)) return true;

    if (this.txindex) return this.txindex.hasTX(hash);
  ...
  async getBlockFilter(hash, filterType) {
    ...
    if (typeof hash === "number") hash = await this.chain.getHash(hash);
