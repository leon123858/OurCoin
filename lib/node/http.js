...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get("/mempool", async (req, res) => {
      ...
      for (const hash of hashes) result.push(util.revHex(hash));
  ...
  handleAuth(socket) {
    ...
    socket.hook("estimate fee", (...args) => {
      ...
      if (!this.fees) return this.network.feeRate;
  ...
  initSockets() {
    ...
    pool.on("tx", (tx) => {
      ...
      for (const socket of sockets) {
        if (!this.filterTX(socket, tx)) continue;
