...
class RPC extends RPCBase {
  ...
  async stop(args, help) {
    if (help || args.length !== 0) throw new RPCError(errs.MISC_ERROR, "stop");
  ...
  async addNode(args, help) {
    ...
    switch (cmd) {
      case "add": {
        this.pool.hosts.addNode(node); // fall through
      }
      case "onetry": {
  ...
  async getAddedNodeInfo(args, help) {
    ...
    for (const node of hosts.nodes) {
      if (target) {
        if (node.host !== target.host) continue;

        if (node.port !== target.port) continue;
      }
  ...
  async ping(args, help) {
    if (help || args.length !== 0) throw new RPCError(errs.MISC_ERROR, "ping");
  ...
  async getNodeAddresses(args, help) {
    ...
    if (count !== 0) count = Math.min(count, maxHost);
    else count = maxHost;
  ...
  async getBlockchainInfo(args, help) {
    ...
    return {
      chain: this.network.type !== "testnet" ? this.network.type : "test",
      blocks: this.chain.height,
  ...
  async getBlock(args, help) {
    ...
    if (!hash) throw new RPCError(errs.TYPE_ERROR, "Invalid block hash.");
    ...
    if (!verbose) return block.toRaw().toString("hex");
  ...
  async getBlockHash(args, help) {
    ...
    if (!hash) throw new RPCError(errs.MISC_ERROR, "Not found.");
  ...
  async getBlockHeader(args, help) {
    ...
    if (!hash) throw new RPCError(errs.MISC_ERROR, "Invalid block hash.");
  ...
  async getBlockFilter(args, help) {
    ...
    if (!filter) throw new RPCError(errs.MISC_ERROR, "Block filter not found.");
  ...
  async getMempoolAncestors(args, help) {
    ...
    if (!hash) throw new RPCError(errs.TYPE_ERROR, "Invalid TXID.");
    ...
    if (verbose) {
      for (const entry of entries) out.push(this.entryToJSON(entry));
    } else {
      for (const entry of entries) out.push(entry.txid());
    }
  ...
  async getTXOut(args, help) {
    ...
    if (!coin) coin = await this.chain.getCoin(hash, index);
  ...
  async getTXOutProof(args, help) {
    ...
    if (hash) {
      ...
      if (tx) block = await this.chain.getBlock(tx.block);
    } else {
      const coin = await this.chain.getCoin(last, 0);
      if (coin) block = await this.chain.getBlock(coin.height);
    }

    if (!block) throw new RPCError(errs.MISC_ERROR, "Block not found.");
  ...
  async verifyTXOutProof(args, help) {
    ...
    if (!data) throw new RPCError(errs.TYPE_ERROR, "Invalid hex string.");
    ...
    for (const hash of tree.matches) out.push(util.revHex(hash));
  ...
  async _submitWork(data) {
    ...
    if (
      header.prevBlock !== attempt.prevBlock ||
      header.bits !== attempt.bits
    ) {
      return false;
    }

    if (!header.verify()) return false;
    ...
    if (!proof.verify(attempt.target)) return false;
    ...
    if (!entry) {
      this.logger.warning(
        "RPC block rejected: %h (bad-prevblk).",
        block.hash()
      );
      return false;
    }
  ...
  async getWork(args, help) {
    ...
    if (args.length === 1) {
      ...
      if (!data) throw new RPCError(errs.TYPE_ERROR, "Invalid work data.");
  ...
  async getBlockTemplate(args, help) {
    ...
    if (mode === "proposal") {
      ...
      if (!data) throw new RPCError(errs.TYPE_ERROR, "Missing data parameter.");
      ...
      try {
        ...
        if (e.type === "VerifyError") return e.reason;
        throw e;
      }
    ...
    if (!this.network.selfConnect) {
      ...
      if (!this.chain.synced) {
        throw new RPCError(
          errs.CLIENT_IN_INITIAL_DOWNLOAD,
    ...
    if (lpid) await this.handleLongpoll(lpid);
  ...
  async getMiningInfo(args, help) {
    ...
    if (attempt) {
      ...
      size = 1000;
      for (const item of attempt.items) size += item.tx.getBaseSize();
    }
  ...
  async verifyBlock(args, help) {
    ...
    if (!data) throw new RPCError(errs.TYPE_ERROR, "Invalid block hex.");
  ...
  async createRawTransaction(args, help) {
    ...
    for (const key of Object.keys(sendTo)) {
      if (key === "data") {
        ...
        if (!value) throw new RPCError(errs.TYPE_ERROR, "Invalid nulldata..");
  ...
  async getRawTransaction(args, help) {
    ...
    if (!meta) throw new RPCError(errs.MISC_ERROR, "Transaction not found.");

    const tx = meta.tx;

    if (!verbose) return tx.toRaw().toString("hex");

    let entry;
    if (meta.block) entry = await this.chain.getEntry(meta.block);
  ...
  async signRawTransaction(args, help) {
    ...
    if (prevout) {
      for (const prev of prevout) {
        ...
        if (keys.length === 0 || !redeemRaw) continue;

        if (!script.isScripthash() && !script.isWitnessScripthash()) continue;
  ...
  async createMultisig(args, help) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (!key) throw new RPCError(errs.TYPE_ERROR, "Invalid key.");
  ...
  async createWitnessAddress(args, help) {
    ...
    if (!raw) throw new RPCError(errs.TYPE_ERROR, "Invalid script hex.");
  ...
  async setMockTime(args, help) {
    ...
    if (time == null) throw new RPCError(errs.TYPE_ERROR, "Invalid timestamp.");
  ...
  async handleLongpoll(lpid) {
    ...
    if (!this.chain.tip.hash.equals(hash)) return;
  ...
  refreshBlock() {
    ...
    for (const job of pollers) job.resolve();
  }
  ...
  bindChain() {
    ...
    this.node.on("tx", () => {
      ...
      if (util.now() - this.lastActivity > 10) this.refreshBlock();
    });
  }
  ...
  async getHashRate(lookup, height) {
    ...
    if (height != null) tip = await this.chain.getEntry(height);
    ...
    for (let i = 0; i < lookup; i++) {
      ...
      if (!entry) throw new RPCError(errs.DATABASE_ERROR, "Not found.");
  ...
  async _executeContractOutput(output, tx) {
    const st = new Stack();
    const script = output.script;
    if (script.isHaveContract()) {
      await script.execute(st, null, tx);
    }
  }

  async _executeContract(block) {
    const promiseList = [];
    for (let tx of block.txs) {
      for (let output of tx.outputs) {
        promiseList.push(this._executeContractOutput(output, tx));
      }
    }
  ...
  async _mineBlocks(blocks, addr, tries) {
    ...
    for (let i = 0; i < blocks; i++) {
      ...
      await this._executeContract(block);
  ...
  async findFork(entry) {
    while (entry) {
      if (await this.chain.isMainChain(entry)) return entry;
      entry = await this.chain.getPrevious(entry);
    }
...
function parseIP(addr, network) {
  try {
    ...
    throw new RPCError(
      errs.CLIENT_INVALID_IP_OR_SUBNET,
