...
class Node extends EventEmitter {
  ...
  constructor(module, config, file, options) {
    ...
    if (options.file || options.config) this.config.open(config);
  ...
  _init(file) {
    ...
    logger.set({
      filename:
        !this.memory && config.bool("log-file") ? config.location(file) : null,
  ...
  async ensure() {
    if (fs.unsupported) return undefined;

    if (this.memory) return undefined;

    if (this.blocks) await this.blocks.ensure();
  ...
  use(plugin) {
    ...
    assert(
      !instance.open || typeof instance.open === "function",
      "`open` must be a function."
    );
    assert(
      !instance.close || typeof instance.close === "function",
