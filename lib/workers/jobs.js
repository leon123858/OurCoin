...
jobs.execute = async function execute(p) {
  try {
    return await jobs.handle(p);
  } catch (e) {
...
jobs.handle = async function handle(p) {
  switch (p.cmd) {
    case packets.types.CHECK:
      return await jobs.check(p.tx, p.view, p.flags);
    case packets.types.CHECKINPUT:
      return await jobs.checkInput(p.tx, p.index, p.coin, p.flags);
 ...
 * @returns {Promise<CheckResultPacket>}
 */

jobs.check = async function check(tx, view, flags) {
  try {
    await tx.check(view, flags);
  } catch (err) {
    if (err.type === "ScriptError") return new packets.CheckResultPacket(err);
    throw err;
  }
 ...
 * @param {Output} coin
 * @param {VerifyFlags} flags
 * @returns {Promise<CheckInputResultPacket>}
 */

jobs.checkInput = async function checkInput(tx, index, coin, flags) {
  try {
    await tx.checkInput(index, coin, flags);
  } catch (err) {
