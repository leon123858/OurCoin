...
class CheckPacket extends Packet {
  ...
  fromRaw(data) {
    ...
    if (this.flags === -1) this.flags = null;
...
class CheckResultPacket extends Packet {
  ...
  fromRaw(data) {
    ...
    if (err.op === 0xff) err.op = -1;

    if (err.ip === 0xffffffff) err.ip = -1;
...
class SignPacket extends Packet {
  ...
  getSize() {
    ...
    for (const ring of this.rings) size += ring.getSize();
  ...
  toWriter(bw) {
    ...
    for (const ring of this.rings) ring.toWriter(bw);
...
class MineResultPacket extends Packet {
  ...
  fromRaw(data) {
    const br = bio.read(data, true);
    this.nonce = -1;
    if (br.readU8() === 1) this.nonce = br.readU32();
    return this;
  }
