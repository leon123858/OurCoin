...
class Wallet extends EventEmitter {
  ...
  async init(options, passphrase) {
    if (passphrase) await this.master.encrypt(passphrase);
  ...
  async setPassphrase(passphrase, old) {
    if (old != null) await this.decrypt(old);
  ...
  async _retoken(passphrase) {
    if (passphrase) await this.unlock(passphrase);
  ...
  async _renameAccount(acct, name) {
    if (!common.isName(name)) throw new Error("Bad account name.");
  ...
  getToken(nonce) {
    if (!this.master.key) throw new Error("Cannot derive token.");
  ...
  async _createAccount(options, passphrase) {
    ...
    if (await this.hasAccount(name)) throw new Error("Account already exists.");
    ...
    if (this.accountDepth === 1) this.increment(b);
  ...
  getAccountIndex(acct) {
    ...
    if (typeof acct === "number") return acct;
  ...
  async getPaths(acct) {
    if (acct != null) return this.getAccountPaths(acct);
  ...
  async _importKey(acct, ring, passphrase) {
    ...
    if (await this.getPath(hash)) throw new Error("Key already exists.");
  ...
  async _importAddress(acct, address) {
    ...
    if (await this.getPath(address)) throw new Error("Address already exists.");
  ...
  async _fund(mtx, options) {
    ...
    if (!change) throw new Error("Account not found.");

    let rate = options.rate;
    if (rate == null) rate = await this.wdb.estimateFee(options.blocks);
  ...
  async createTX(options, force) {
    ...
    for (const obj of outputs) {
      ...
      if (output.isDust() && !obj.contract) throw new Error("Output is dust.");

      if (!obj.contract && output.value > 0) {
        if (!addr) throw new Error("Cannot send to unknown address.");

        if (addr.isNull()) throw new Error("Cannot send to null address.");
      }
    ...
    assert(
      mtx.verifyInputs(this.wdb.state.height + 1),
      "TX failed context check."
    );
    if (options.template === false) return mtx;

    const total = await this.template(mtx);

    if (total === 0) throw new Error("Templating failed.");
  ...
  async _send(options, passphrase) {
    ...
    if (!mtx.isSigned()) throw new Error("TX could not be fully signed.");
  ...
  async increaseFee(hash, rate, passphrase) {
    ...
    if (!wtx) throw new Error("Transaction not found.");

    if (wtx.height !== -1) throw new Error("Transaction is confirmed.");

    const tx = wtx.tx;

    if (tx.isCoinbase()) throw new Error("Transaction is a coinbase.");

    const view = await this.getSpentView(tx);

    if (!tx.hasCoins(view)) throw new Error("Not all coins available.");

    const oldFee = tx.getFee(view);

    let fee = tx.getMinFee(null, rate);

    if (fee > CoinSelector.MAX_FEE) fee = CoinSelector.MAX_FEE;
    ...
    if (!change) throw new Error("No change output.");

    change.value += oldFee;

    if (mtx.getFee() !== 0) throw new Error("Arithmetic error for change.");

    change.value -= fee;

    if (change.value < 0) throw new Error("Fee is too high.");
  ...
  async resend() {
    ...
    for (const wtx of wtxs) txs.push(wtx.tx);

    const sorted = common.sortDeps(txs);

    for (const tx of sorted) await this.wdb.send(tx);
  ...
  async getInputPaths(mtx) {
    ...
    if (!mtx.hasCoins()) throw new Error("Not all coins available.");
  ...
  async _getPendingAncestors(tx, set) {
    for (const { prevout } of tx.inputs) {
      ...
      if (set.size > this.maxAncestors) break;
  ...
  async getSmartCoins(acct) {
    ...
    for (const credit of credits) {
      ...
      if (this.txdb.isLocked(coin)) continue;
