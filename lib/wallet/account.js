...
class Account {
  ...
  fromOptions(options) {
    ...
    assert(
      HDPublicKey.isHDPublicKey(options.accountKey),
      "Account key is required."
    );
    assert(
      options.accountIndex >>> 0 === options.accountIndex,
    ...
    if (options.keys) {
      ...
      for (const key of options.keys) this.pushKey(key);
    }
  ...
  async setLookahead(b, lookahead) {
    if (lookahead === this.lookahead) return;
    ...
    if (lookahead < this.lookahead) {
      ...
      if (this.witness) this.nestedDepth += diff;
  ...
  toRaw() {
    ...
    if (this.initialized) flags |= 1;

    if (this.witness) flags |= 2;
    ...
    for (const key of this.keys) writeKey(key, bw);
...
Account.typesByVal = ["PUBKEYHASH", "MULTISIG"];
