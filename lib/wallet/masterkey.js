...
class MasterKey {
  ...
  async _decrypt(passphrase, clean) {
    if (!this.encrypted) throw new Error("Master key is not encrypted.");
  ...
  async _encrypt(passphrase, clean) {
    if (this.encrypted) throw new Error("Master key is already encrypted.");
...
MasterKey.algByVal = ["PBKDF2", "SCRYPT"];
