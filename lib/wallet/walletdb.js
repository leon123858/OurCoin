...
const { ChainState, BlockMeta, TXRecord, MapRecord } = records;
...
class WalletDB extends EventEmitter {
  ...
  async open() {
    ...
    if (this.options.wipeNoReally) await this.wipe();
  ...
  async syncState() {
    ...
    if (cache) {
      if (!(await this.getBlock(0))) return this.migrateState(cache);
  ...
  async syncChain() {
    ...
    for (;;) {
      ...
      if (await this.client.getEntry(tip.hash)) break;
  ...
  async estimateFee(blocks) {
    if (this.feeRate > 0) return this.feeRate;

    const json = await this.client.estimateFee(blocks);

    if (!json) throw new Error("Fee not found.");

    if (!Number.isInteger(json.rate)) throw new Error("Fee is not an integer.");

    if (json.rate < this.network.feeRate) return this.network.feeRate;

    if (json.rate > this.network.maxFeeRate) return this.network.maxFeeRate;
  ...
  addFilter(data) {
    if (!this.filterSent) return undefined;
  ...
  async ensureWID(id) {
    if (typeof id === "number") {
      if (!(await this.db.has(layout.W.encode(id)))) return -1;
      return id;
    }
  ...
  async _rename(wallet, id) {
    if (!common.isName(id)) throw new Error("WDB: Bad wallet ID.");

    if (await this.has(id)) throw new Error("WDB: ID not available.");
  ...
  async _remove(wid) {
    ...
    if (id === "primary") throw new Error("Cannot remove primary wallet.");
  ...
  async resend() {
    ...
    for (const wid of wids) await this.resendPending(wid);
  }
  ...
  async resendPending(wid) {
    ...
    for (const hash of hashes) {
      ...
      if (wtx.tx.isCoinbase()) continue;

      txs.push(wtx.tx);
    }

    for (const tx of common.sortDeps(txs)) await this.send(tx);
  }
  ...
  async getWalletsByTX(tx) {
    ...
    if (!tx.isCoinbase()) {
      for (const { prevout } of tx.inputs) {
        ...
        if (!this.testFilter(prevout.toRaw())) continue;

        const map = await this.getOutpointMap(hash, index);

        if (!map) continue;

        for (const wid of map.wids) wids.add(wid);
      }
    }
  ...
  async getTip() {
    ...
    if (!tip) throw new Error("WDB: Tip not found!");
  ...
  async _addBlock(entry, txs) {
    ...
    try {
      ...
      for (const tx of txs) {
        if (await this._addTX(tx, tip)) total += 1;
      }
