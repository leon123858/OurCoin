...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add("deploycontract", this.deployContract);
    this.add("callcontract", this.callContract);
    this.add("transfercontract", this.transferContract);
  }
  ...
  async resendWalletTransactions(args, help) {
    ...
    for (const tx of txs) hashes.push(tx.txid());
  ...
  async dumpPrivKey(args, help) {
    ...
    if (!ring) throw new RPCError(errs.MISC_ERROR, "Key not found.");
  ...
  async dumpWallet(args, help) {
    ...
    if (!file) throw new RPCError(errs.TYPE_ERROR, "Invalid parameter.");
    ...
    const out = [
      ...
      format(
        "# * Best block at time of backup was %d (%s).",
        tip.height,
  ...
  async getAddressesByAccount(args, help) {
    ...
    try {
      ...
      if (e.message === "Account not found.") return [];
      throw e;
    }
  ...
  async getNewAddress(args, help) {
    ...
    if (name === "" || args.length === 0) name = "default";
  ...
  async getReceivedByAccount(args, help) {
    ...
    for (const path of paths) filter.add(path.hash);
  ...
  async getReceivedByAddress(args, help) {
    ...
    for (const wtx of txs) {
      if (wtx.getDepth(height) < minconf) continue;
      ...
      for (const output of wtx.tx.outputs) {
        ...
        if (addr.getHash().equals(hash)) total += output.value;
      }
    }
  ...
  async _toWalletTX(wtx) {
    ...
    if (!details) throw new RPCError(errs.WALLET_ERROR, "TX not found.");
    ...
    for (let i = 0; i < details.outputs.length; i++) {
      ...
      if (member.path) {
        if (member.path.branch === 1) continue;
  ...
  async getTransaction(args, help) {
    ...
    if (!hash) throw new RPCError(errs.TYPE_ERROR, "Invalid parameter");

    const wtx = await wallet.getTX(hash);

    if (!wtx) throw new RPCError(errs.WALLET_ERROR, "TX not found.");
  ...
  async importPrivKey(args, help) {
    ...
    if (rescan) await this.wdb.rescan(0);
  ...
  async importWallet(args, help) {
    ...
    for (const key of keys) await wallet.importKey(0, key);
  ...
  async importAddress(args, help) {
    ...
    if (p2sh) {
      ...
      if (!script) throw new RPCError(errs.TYPE_ERROR, "Invalid parameters.");
    ...
    try {
      ...
      if (e.message !== "Address already exists.") throw e;
    }
  ...
  async importPubkey(args, help) {
    ...
    if (!data) throw new RPCError(errs.TYPE_ERROR, "Invalid parameter.");
  ...
  async _listReceived(minconf, empty, watchOnly, account) {
    ...
    const result = [];
    for (const entry of out) {
      if (!empty && entry.amount === 0) continue;
  ...
  async listSinceBlock(args, help) {
    ...
    if (wallet.watchOnly !== watchOnly) return [];
    ...
    if (block) {
      ...
      else throw new RPCError(errs.MISC_ERROR, "Block not found.");
    }

    if (height === -1) height = chainHeight;
    ...
    for (const wtx of txs) {
      if (wtx.height < height) continue;

      if (wtx.getDepth(chainHeight) < minconf) continue;
  ...
  async _toListTX(wtx) {
    ...
    if (wtx.height === -1 && wtx.tx.isRBF()) rbf = true;
  ...
  async lockUnspent(args, help) {
    ...
    if (!outputs) throw new RPCError(errs.TYPE_ERROR, "Invalid parameter.");
  ...
  async setTXFee(args, help) {
    ...
    if (rate == null) throw new RPCError(errs.TYPE_ERROR, "Invalid parameter.");
  ...
  async signMessage(args, help) {
    ...
    if (!ring) throw new RPCError(errs.WALLET_ERROR, "Address not found.");
  ...
  async importPrunedFunds(args, help) {
    ...
    if (!entry) throw new RPCError(errs.VERIFY_ERROR, "Invalid proof.");
  ...
  async selectWallet(args, help) {
    ...
    if (!wallet) throw new RPCError(errs.WALLET_ERROR, "Wallet not found.");
  ...
  async deployContract(args, help) {
    if (help || args.length < 1) {
      throw new RPCError(
        errs.MISC_ERROR,
        "deploycontract [string of contract code] ([ account ])"
      );
    }
    const uuid = uid();
    const wallet = this.wallet;
    const valid = new Validator(args);
    const str = valid.str(0);
    let name = valid.str(1);

    if (name === "" || args.length === 1) name = "default";
    const addr = await wallet.createReceive(name);
    const addressKey = addr.getAddress("string", this.network);
      ...
      outputs: [
        {
          contract: true,
          script: new Script([
            Opcode.fromString(addressKey),
            Opcode.fromString(uuid),
            Opcode.fromString(str),
    ...
    await wallet.send(options);
    return uuid;
  }

  async callContract(args, help) {
    if (help || args.length < 2) {
      throw new RPCError(
        errs.MISC_ERROR,
        "callcontract [uid of contract] [string of parameters json]"
      );
    }

    const wallet = this.wallet;
    const valid = new Validator(args);
    const uid = valid.str(0);
    const parameters = valid.get(1);
    assert(
      typeof parameters == "string" || typeof parameters == "object",
      "Second argument should by a string which is converted by json"
    );
    const options = {
      outputs: [
        {
  ...
  async transferContract(args, help) {
    if (help || args.length < 2) {
      throw new RPCError(
        errs.MISC_ERROR,
        "transfercontract [uid of contract] [new Owner address]"
      );
    }
