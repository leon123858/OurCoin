...
class CoinSelector {
  ...
  async selectEstimate() {
    ...
    do {
      ...
      if (!this.isFull()) index = this.fund(index);
  ...
  async attemptSelection() {
    ...
    const changeSpendingSize = await changeOutputCoin.estimateSpendingSize(
      this.getAccount
    );
    ...
    if (bestSelection && bestSelection.length > 0) {
      ...
      if (isChange) this.tx.outputs.push(change);
      // update fee
  ...
  selectSRD(target) {
    ...
    while (index < arr.length) {
      ...
      if (selectedValue >= target) return selected;
      index++;
    }
  ...
  selectBnB(target, costOfChange) {
    ...
    if (remainingValue < target) {
      throw new FundingError("Not enough funds.", remainingValue, target);
    }
    ...
    while (tries-- > 0) {
      ...
      if (
        currValue + remainingValue < target ||
      ...
      if (backtrack) {
        ...
        if (
          ...
          pointer.effectiveValue !== this.coinPointers[index - 1].effectiveValue
        ) {
          selected.push(index);
          currValue += this.coinPointers[index].effectiveValue;
        }
  ...
  selectLowestLarger(target) {
    ...
    while (target >= effectiveValue) {
      ...
      if (index === this.coinPointers.length) break;
  ...
  getWaste(selected, costOfChange, target) {
    ...
    for (const i of selected) {
      ...
      waste +=
        (currentFeeRate - CoinSelector.LONG_TERM_FEERATE) *
