...
class TXDB {
  ...
  async add(tx, block) {
    ...
    if (existing) {
      ...
      if (existing.height !== -1) return null;
    ...
    if (!block) {
      ...
      if (!(await this.removeConflicts(tx, true))) return null;
    } else {
      // Potentially remove double-spenders.
  ...
  async insert(wtx, block) {
    ...
    if (!tx.isCoinbase()) {
      ...
      for (let i = 0; i < tx.inputs.length; i++) {
        ...
        if (!credit) {
          ...
          if (!block) await this.writeInput(b, tx, i);
          continue;
        }
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      if (block) state.confirmed(path, output.value);
    ...
    if (!block) b.put(layout.p.encode(hash), null);
    else b.put(layout.h.encode(height, hash), null);
    ...
    for (const [acct, delta] of state.accounts) {
      ...
      if (!block) b.put(layout.P.encode(acct, hash), null);
      else b.put(layout.H.encode(acct, height, hash), null);
    }
  ...
  async confirm(wtx, block) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      if (credit.spent) await this.updateSpentCoin(b, tx, i, height);
  ...
  async erase(wtx, block) {
    ...
    if (!tx.isCoinbase()) {
      ...
      for (let i = 0; i < tx.inputs.length; i++) {
        ...
        if (!credit) {
          if (!block) await this.removeInput(b, tx, i);
          continue;
        }
        ...
        if (block) state.confirmed(path, coin.value);
    ...
    if (!block) b.del(layout.p.encode(hash));
    else b.del(layout.h.encode(height, hash));
    ...
    for (const [acct, delta] of state.accounts) {
      ...
      if (!block) b.del(layout.P.encode(acct, hash));
      else b.del(layout.H.encode(acct, height, hash));
    }
  ...
  async removeRecursive(wtx) {
    ...
    if (!(await this.hasTX(hash))) return null;
  ...
  async removeConflicts(tx, conf) {
    ...
    for (const { prevout } of tx.inputs) {
      ...
      // Did _we_ spend it?
      if (spent.hash.equals(txid)) continue;

      const spender = await this.getTX(spent.hash);
      assert(spender);

      if (conf && spender.height !== -1) return false;
  ...
  lockTX(tx) {
    ...
    for (const input of tx.inputs) this.lockCoin(input.prevout);
  }
  ...
  unlockTX(tx) {
    ...
    for (const input of tx.inputs) this.unlockCoin(input.prevout);
  }
  ...
  unlockCoins() {
    for (const coin of this.getLocked()) this.unlockCoin(coin);
  }
  ...
  getLocked() {
    ...
    for (const key of this.locked.keys()) outpoints.push(Outpoint.fromKey(key));
  ...
  getHistoryHashes(acct) {
    ...
    if (acct !== -1) return this.getAccountHistoryHashes(acct);
  ...
  getPendingHashes(acct) {
    ...
    if (acct !== -1) return this.getAccountPendingHashes(acct);
  ...
  getOutpoints(acct) {
    ...
    if (acct !== -1) return this.getAccountOutpoints(acct);
  ...
  getHeightRangeHashes(acct, options) {
    ...
    if (acct !== -1) return this.getAccountHeightRangeHashes(acct, options);
  ...
  getRangeHashes(acct, options) {
    ...
    if (acct !== -1) return this.getAccountRangeHashes(acct, options);
  ...
  getHistory(acct) {
    ...
    // Slow case
    if (acct !== -1) return this.getAccountHistory(acct);
  ...
  getCredits(acct) {
    ...
    // Slow case
    if (acct !== -1) return this.getAccountCredits(acct);
  ...
  async getSpentCredits(tx) {
    ...
    for (let i = 0; i < tx.inputs.length; i++) credits.push(null);
  ...
  async getUnspentCredits(acct) {
    ...
    if (acct !== -1) return this.getUnspentAccountCredits(acct);
  ...
  async toDetails(wtxs) {
    ...
    if (!Array.isArray(wtxs)) return this._toDetails(wtxs);
  ...
  async _toDetails(wtx) {
    ...
    for (let i = 0; i < tx.inputs.length; i++) {
      ...
      if (coin) path = await this.getPath(coin);
  ...
  async getSpentCoin(spent, prevout) {
    const data = await this.bucket.get(
      layout.d.encode(spent.hash, spent.index)
    );
  ...
  async getBalance(acct) {
    ...
    if (acct !== -1) return this.getAccountBalance(acct);
  ...
  async getAccountBalance(acct) {
    ...
    if (!data) return new Balance(acct);
  ...
  async abandon(hash) {
    ...
    if (!result) throw new Error("TX not eligible.");
...
class Details {
  ...
  getFee() {
    ...
    for (const output of this.outputs) outputValue += output.value;
...
class BlockRecord {
  ...
  toArray() {
    const hashes = [];
    for (const hash of this.hashes) hashes.push(hash);
    return hashes;
  }
