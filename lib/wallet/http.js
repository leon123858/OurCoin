...
class HTTP extends Server {
  ...
  init() {
    ...
    this.on("listening", (address) => {
      this.logger.info(
        "Wallet HTTP server listening on %s (port=%d).",
        address.address,
        address.port
      );
    });
  ...
  initRouter() {
    ...
    this.put("/wallet/:id", async (req, res) => {
      ...
      if (master) master = HDPrivateKey.fromBase58(master, this.network);

      if (mnemonic) mnemonic = Mnemonic.fromPhrase(mnemonic);
    ...
    this.post("/wallet/:id/send", async (req, res) => {
      ...
      for (const output of outputs) {
        ...
        if (addr) addr = Address.fromString(addr, this.network);

        if (script) script = Script.fromRaw(script);
    ...
    this.post("/wallet/:id/create", async (req, res) => {
      ...
      if (sign) await req.wallet.sign(tx, passphrase);
    ...
    this.get("/wallet/:id/coin", async (req, res) => {
      ...
      for (const coin of coins) result.push(coin.getJSON(this.network));
    ...
    this.get("/wallet/:id/locked", async (req, res) => {
      ...
      for (const outpoint of locked) result.push(outpoint.toJSON());
  ...
  initSockets() {
    const handleTX = (event, wallet, tx, details) => {
      ...
      if (!this.channel(name) && !this.channel("w:*")) return;

      const json = details.toJSON(this.network, this.wdb.liveHeight());

      if (this.channel(name)) this.to(name, event, wallet.id, json);

      if (this.channel("w:*")) this.to("w:*", event, wallet.id, json);
    };
    ...
    this.wdb.on("balance", (wallet, balance) => {
      ...
      if (this.channel(name)) this.to(name, "balance", wallet.id, json);

      if (this.channel("w:*")) this.to("w:*", "balance", wallet.id, json);
    });
    ...
    this.wdb.on("address", (wallet, receive) => {
      ...
      for (const addr of receive) json.push(addr.toJSON(this.network));

      if (this.channel(name)) this.to(name, "address", wallet.id, json);

      if (this.channel("w:*")) this.to("w:*", "address", wallet.id, json);
    });
  }
  ...
  handleAuth(socket) {
    socket.hook("join", async (...args) => {
      ...
      if (!this.options.walletAuth) {
        ...
        if (safeEqual(token, this.options.adminToken)) socket.join("admin");
      }
