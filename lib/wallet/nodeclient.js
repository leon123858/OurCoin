...
class NodeClient extends AsyncEmitter {
  ...
  async getEntry(hash) {
    ...
    if (!(await this.node.chain.isMainChain(entry))) return null;
  ...
  async estimateFee(blocks) {
    if (!this.node.fees) return { rate: this.network.feeRate };
  ...
  async rescan(start) {
    if (this.node.spv) return this.node.chain.reset(start);
