...
class AddrIndexer {
  ...
  get(addr, options = {}) {
    ...
    for (const entry of values) out.push(entry.tx);
  ...
  remove(hash) {
    ...
    for (const addr of addrs) {
      ...
      if (items.size === 0) this.index.delete(key);
    }
