...
class Mempool extends EventEmitter {
  ...
  async _addBlock(block, txs) {
    ...
    for (let i = txs.length - 1; i >= 1; i--) {
      ...
      if (!entry) {
        ...
        if (this.waiting.has(hash)) await this.handleOrphans(tx);
        continue;
      }
  ...
  async _removeBlock(block, txs) {
    ...
    for (let i = 1; i < txs.length; i++) {
      ...
      if (this.hasEntry(hash)) continue;
  ...
  hasCoin(hash, index) {
    ...
    if (this.isSpent(hash, index)) return false;

    if (index >= entry.tx.outputs.length) return false;
  ...
  exists(hash) {
    if (this.locker.pending(hash)) return true;
  ...
  async _addTX(tx, id) {
    ...
    try {
      ...
      if (err.type === "VerifyError") {
        if (!tx.hasWitness() && !err.malleated) this.rejects.add(tx.hash());
      }
      throw err;
    }
  ...
  async insertTX(tx, id) {
    ...
    if (!valid) throw new VerifyError(tx, "invalid", reason, score);
    ...
    if (this.options.requireStandard) {
      ...
      if (!valid) throw new VerifyError(tx, "nonstandard", reason, score);
  ...
  async verify(entry, view) {
    ...
    if (fee === -1) throw new VerifyError(tx, "invalid", reason, score);
    ...
    try {
      ...
      if (!(await this.verifyResult(tx, view, flags))) throw err;
    ...
    if (this.options.paranoidChecks) {
      ...
      assert(
        await this.verifyResult(tx, view, flags),
  ...
  async verifyInputs(tx, view, flags) {
    if (await tx.verifyAsync(view, flags, this.workers)) return;
  ...
  getHistory() {
    ...
    for (const entry of this.map.values()) txs.push(entry.tx);
  ...
  maybeOrphan(tx, view, id) {
    ...
    for (const prev of hashes.keys()) {
      if (!this.waiting.has(prev)) this.waiting.set(prev, new BufferSet());
  ...
  async _getSpentView(tx) {
    ...
    for (const { prevout } of tx.inputs) {
      ...
      if (tx) {
        if (index < tx.outputs.length) view.addIndex(tx, index, -1);
        continue;
      }
  ...
  async getCoinView(tx) {
    ...
    for (const { prevout } of tx.inputs) {
      ...
      if (tx) {
        if (this.hasCoin(hash, index)) view.addIndex(tx, index, -1);
        continue;
      }
  ...
  getSnapshot() {
    ...
    for (const hash of this.map.keys()) keys.push(hash);
...
class MempoolCache {
  ...
  constructor(options) {
    ...
    if (options.persistent) this.db = bdb.create(options);
  }
