...
class ConfirmStats {
  ...
  estimateMedian(target, needed, breakpoint, greater, height) {
    ...
    for (let i = start; i >= 0 && i <= max; i += step) {
      ...
      if (total >= needed / (1 - this.decay)) {
        ...
        if (greater && perc < breakpoint) break;
    ...
    for (let i = minBucket; i <= maxBucket; i++) sum += this.txAvg[i];
  ...
  toRaw() {
    ...
    for (let i = 0; i < this.maxConfirms; i++) writeArray(bw, this.confAvg[i]);
  ...
  fromRaw(data) {
    ...
    for (let i = 0; i < maxConfirms; i++) confAvg[i] = readArray(br);
...
class PolicyEstimator {
  ...
  constructor(logger) {
    ...
    if (policy.MIN_RELAY >= MIN_FEERATE) this.minTrackedFee = policy.MIN_RELAY;
  ...
  init() {
    ...
    for (let b = minFee; b <= MAX_FEERATE; b *= FEE_SPACING) fee.push(b);

    fee.push(INF_FEERATE);

    const priority = [];

    for (let b = minPri; b <= MAX_PRIORITY; b *= PRI_SPACING) priority.push(b);
  ...
  isFeePoint(fee, priority) {
    if (
      (priority < this.minTrackedPri && fee >= this.minTrackedFee) ||
  ...
  isPriPoint(fee, priority) {
    if (
      (fee < this.minTrackedFee && priority >= this.minTrackedPri) ||
  ...
  processBlock(height, entries, current) {
    ...
    if (this.feeLikely === -1) this.feeLikely = INF_FEERATE;
    ...
    if (this.feeUnlikely === -1) this.feeUnlikely = 0;
    ...
    if (this.priLikely === -1) this.priLikely = INF_PRIORITY;
    ...
    if (this.priUnlikely === -1) this.priUnlikely = 0;

    this.feeStats.clearCurrent(height);
    this.priStats.clearCurrent(height);

    for (const entry of entries) this.processBlockTX(height, entry);

    this.feeStats.updateAverages();
    this.priStats.updateAverages();

    this.logger.debug(
      "Done updating estimates" +
  ...
  estimateFee(target, smart) {
    ...
    if (smart == null) smart = true;

    assert(target >>> 0 === target, "Target must be a number.");
    assert(
      target <= this.feeStats.maxConfirms,
  ...
  estimatePriority(target, smart) {
    ...
    assert(target >>> 0 === target, "Target must be a number.");
    assert(
      target <= this.priStats.maxConfirms,
      "Too many confirmations for estimate."
    );
...
function writeArray(bw, buckets) {
  ...
  for (let i = 0; i < buckets.length; i++) bw.writeDouble(buckets[i]);
}

function readArray(br) {
  const buckets = new Float64Array(br.readVarint());

  for (let i = 0; i < buckets.length; i++) buckets[i] = br.readDouble();
