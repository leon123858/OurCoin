...
class LevelBlockStore extends AbstractBlockStore {
  ...
  async read(hash, offset, length) {
    ...
    if (offset) {
      if (offset + length > raw.length) throw new Error("Out-of-bounds read.");
  ...
  async pruneMerkle(hash) {
    if (!(await this.hasMerkle(hash))) return false;
  ...
  async pruneUndo(hash) {
    if (!(await this.hasUndo(hash))) return false;
  ...
  async pruneFilter(hash, filterType) {
    if (!(await this.hasFilter(hash, filterType))) return false;
