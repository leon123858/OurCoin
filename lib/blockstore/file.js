...
class FileBlockStore extends AbstractBlockStore {
  ...
  constructor(options) {
    ...
    assert(
      Number.isSafeInteger(this.maxFileLength),
  ...
  filepath(type, fileno) {
    ...
    if (num.length > pad) throw new Error("File number too large.");
    ...
    if (!prefix) throw new Error("Unknown file prefix.");
  ...
  async allocate(type, length) {
    ...
    if (last) fileno = bio.readU32(last, 0);
  ...
  async _write(type, hash, data) {
    if (this.writing[type]) throw new Error("Already writing.");
    ...
    // of the block data.
    if (type !== types.BLOCK && type !== types.MERKLE) mlength += 32;
    ...
    if (type !== types.BLOCK && type !== types.MERKLE) bwm.writeHash(hash);

    const magic = bwm.render();

    const { fileno, filerecord, filepath } = await this.allocate(type, length);
  ...
  async _read(type, hash, offset, length) {
    ...
    if (!length && offset > 0) length = blockrecord.length - offset;

    if (!length) length = blockrecord.length;
    ...
    if (bytes !== length) throw new Error("Wrong number of bytes read.");
  ...
  async _prune(type, hash) {
    ...
    if (filerecord.blocks === 0) b.del(layout.f.encode(type, blockrecord.file));
    else b.put(layout.f.encode(type, blockrecord.file), filerecord.toRaw());
