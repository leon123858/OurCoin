...
class Indexer extends EventEmitter {
  ...
  async open() {
    ...
    if (data) this.height = bio.readU32(data, 0);
    else await this.saveGenesis();
  ...
  async _syncBlock(meta, block, view) {
    ...
    if (meta && block && view) {
      if (meta.height === this.height + 1) {
        ...
        if (prev.hash.compare(block.prevBlock) !== 0) return false;
        ...
        if (current.hash.compare(block.hash()) !== 0) return false;
  ...
  async _syncChain() {
    ...
    while (height > 0) {
      ...
      if (await this.getEntry(meta.hash)) break;
