...
class TX {
  ...
  fromOptions(options) {
    ...
    if (options.version != null) {
      assert(
        options.version >>> 0 === options.version,
    ...
    if (options.locktime != null) {
      assert(
        options.locktime >>> 0 === options.locktime,
  ...
  frame() {
    ...
    let raw;
    if (this.hasWitness()) raw = this.frameWitness();
    else raw = this.frameNormal();
  ...
  async check(view, flags) {
    ...
    for (let i = 0; i < this.inputs.length; i++) {
      ...
      await this.checkInput(i, coin, flags);
    }
  }
  ...
  async checkInput(index, coin, flags) {
    const input = this.inputs[index];
  ...
  async checkInputAsync(index, coin, flags, pool) {
    ...
    if (!pool) {
      await this.checkInput(index, coin, flags);
      return;
    }
  ...
  async verifyInput(index, coin, flags) {
    try {
      await this.checkInput(index, coin, flags);
    } catch (e) {
      if (e.type === "ScriptError") return false;
  ...
  fromReader(br, block) {
    if (hasWitnessBytes(br)) return this.fromWitnessReader(br, block);
  ...
  writeWitness(bw) {
    ...
    for (const input of this.inputs) input.witness.toWriter(bw);
