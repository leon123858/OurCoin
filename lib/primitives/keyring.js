...
class KeyRing {
  ...
  getNestedHash(enc) {
    ...
    if (!this._nestedHash) this._nestedHash = this.getProgram().hash160();

    return enc === "hex" ? this._nestedHash.toString("hex") : this._nestedHash;
  }
  ...
  getNestedAddress(enc, network) {
    ...
    if (enc === "base58") return this._nestedAddress.toBase58(network);

    if (enc === "string") return this._nestedAddress.toString(network);
  ...
  getScriptHash(enc) {
    if (this.witness) return this.getScriptHash256(enc);
  ...
  getScriptHash256(enc) {
    ...
    if (!this._scriptHash256) this._scriptHash256 = this.script.sha256();
  ...
  getKeyAddress(enc, network) {
    if (!this._keyAddress) {
      ...
      let addr;
      if (this.witness) addr = Address.fromWitnessPubkeyhash(hash);
      else addr = Address.fromPubkeyhash(hash);
  ...
  getHash(enc) {
    if (this.nested) return this.getNestedHash(enc);
  ...
  getAddress(enc, network) {
    if (this.nested) return this.getNestedAddress(enc, network);
  ...
  ownHash(hash) {
    ...
    if (this.witness) {
      if (hash.equals(this.getNestedHash())) return true;
    }
  ...
  getRedeem(hash) {
    if (this.witness) {
      if (hash.equals(this.getNestedHash())) return this.getProgram();
    }

    if (this.script) {
      if (hash.equals(this.getScriptHash160())) return this.script;

      if (hash.equals(this.getScriptHash256())) return this.script;
    }
  ...
  getVersion() {
    if (!this.witness) return -1;

    if (this.nested) return -1;
  ...
  getType() {
    if (this.nested) return Address.types.SCRIPTHASH;

    if (this.witness) return Address.types.WITNESS;
  ...
  toWriter(bw) {
    ...
    if (this.witness) field |= 1;

    if (this.nested) field |= 2;
    ...
    if (this.script) bw.writeVarBytes(this.script.toRaw());
    else bw.writeVarint(0);
