...
class Coin extends Output {
  ...
  fromOptions(options) {
    ...
    if (options.value != null) {
      assert(
        Number.isSafeInteger(options.value) && options.value >= 0,
    ...
    if (options.coinbase != null) {
      assert(
        typeof options.coinbase === "boolean",
  ...
  fromJSON(json) {
    ...
    assert(
      json.height === -1 || json.height >>> 0 === json.height,
      "Height must be a uint32."
    );
    assert(
      Number.isSafeInteger(json.value) && json.value >= 0,
      "Value must be a uint64."
    );
