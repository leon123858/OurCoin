...
class MTX extends TX {
  ...
  fromOptions(options) {
    ...
    if (options.changeIndex != null) {
      if (options.changeIndex !== -1) {
        assert(
          options.changeIndex >>> 0 === options.changeIndex,
  ...
  addOutput(script, value) {
    ...
    if (value != null) output = Output.fromScript(script, value);
    else output = Output.fromOptions(script);
   ...
   * @returns {Promise<Boolean>} Whether the inputs are valid.
   * @throws {ScriptError} on invalid inputs
   */

  async check(flags) {
    return await super.check(this.view, flags);
  }
  ...
  async fund(coins, options) {
    ...
    if (select.subtractFee) {
      ...
      if (index !== -1) this.subtractIndex(index, select.fee);
      else this.subtractFee(select.fee);
    }
