...
function writeJournal(batch, state, hash) {
  ...
  if (!hash) hash = consensus.ZERO_HASH;
...
async function readJournal() {
  ...
  if (!data) return [STATE_VERSION, consensus.ZERO_HASH];

  if (data.length !== 34) throw new Error("Bad migration length.");

  if (data[0] !== MIGRATION_ID) throw new Error("Bad migration id.");
...
async function updateVersion() {
  ...
  if (!raw) throw new Error("No DB version found!");

  const version = raw.readUInt32LE(0, true);

  if (version !== 2) throw Error(`DB is version ${version}.`);
...
async function reserializeUndo(hash) {
  ...
  if (!hash.equals(consensus.ZERO_HASH)) tip = await getEntry(hash);
  ...
  while (tip.height !== 0 && !hasSPV) {
    ...
    if (!blockData) {
      if (!hasPruned) throw new Error(`Block not found: ${tip.hash}.`);
      break;
    }
    ...
    for (let i = block.txs.length - 1; i >= 1; i--) {
      ...
      for (let j = tx.inputs.length - 1; j >= 0; j--) {
        ...
        if (!lruCache.has(prevout.hash)) lruCache.set(prevout.hash, meta);
  ...
  console.log("Reserialized %d undo records (%d coins).", total, totalCoins);

  return [STATE_CLEANUP, consensus.ZERO_HASH];
}

async function cleanupIndex() {
  if (hasSPV) return [STATE_COINS, consensus.ZERO_HASH];
...
async function reserializeCoins(hash) {
  if (hasSPV) return [STATE_ENTRY, consensus.ZERO_HASH];
  ...
  while (start) {
    ...
    if (item.key.length !== 33) continue;
...
async function reserializeEntries(hash) {
  ...
  if (!hash.equals(consensus.ZERO_HASH)) {
    const item = await iter.next();
    if (!item) start = false;
    else assert(item.key.equals(pair("e", hash)));
  }
...
async function isMainChain(entry, tip) {
  if (entry.hash.equals(tip)) return true;

  if (await db.get(pair("n", entry.hash))) return true;
...
function pair(prefix, hash) {
  ...
  if (typeof prefix === "string") prefix = prefix.charCodeAt(0);
  key[0] = prefix;
  write(key, hash, 1);
  return key;
}
...
(async () => {
  ...
  if (await isSPV()) hasSPV = true;

  if (await isPruned()) hasPruned = true;

  if (await isIndexed()) hasIndex = true;

  if (shouldPrune && hasPruned) throw new Error("Database is already pruned.");
  ...
  if (state === STATE_VERSION) [state, hash] = await updateVersion();

  if (state === STATE_UNDO) [state, hash] = await reserializeUndo(hash);

  if (state === STATE_CLEANUP) [state, hash] = await cleanupIndex();

  if (state === STATE_COINS) [state, hash] = await reserializeCoins(hash);

  // if (state === STATE_ENTRY)
  //   [state, hash] = await reserializeEntries(hash);

  if (state === STATE_ENTRY) [state, hash] = [STATE_FINAL, consensus.ZERO_HASH];

  if (state === STATE_FINAL) [state, hash] = await finalize();
