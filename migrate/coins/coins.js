...
function Coins(options) {
  if (!(this instanceof Coins)) return new Coins(options);
...
Coins.prototype.add = function add(index, entry) {
  ...
  while (this.outputs.length <= index) this.outputs.push(null);
...
Coins.prototype.has = function has(index) {
  if (index >= this.outputs.length) return false;
...
Coins.prototype.isUnspent = function isUnspent(index) {
  ...
  if (!output || output.spent) return false;
...
Coins.prototype.length = function length() {
  ...
  while (len > 0 && !this.isUnspent(len - 1)) len--;
...
Coins.prototype.cleanup = function cleanup() {
  ...
  while (len > 0 && !this.outputs[len - 1]) len--;
...
Coins.prototype.toRaw = function toRaw() {
  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < 8 && 2 + i * 8 + j < len; j++) {
      if (this.isUnspent(2 + i * 8 + j)) ch |= 1 << j;
    }
    bw.writeU8(ch);
  }
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (!output || output.spent) continue;
...
Coins.prototype.fromRaw = function fromRaw(data, hash) {
  ...
  if ((code & 2) !== 0) first = CoinEntry.fromReader(br);

  if ((code & 4) !== 0) second = CoinEntry.fromReader(br);
...
CoinEntry.prototype.getSize = function getSize() {
  if (!this.raw) return compress.size(this.output);
