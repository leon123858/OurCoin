...
(async () => {
  ...
  {
    const jobs = [];
    for (let i = 0; i < 1000; i++) jobs.push(wallet.createAccount({}));

    const end = bench("accounts");
    const result = await Promise.all(jobs);
    end(1000);

    for (const addr of result) addrs.push(addr.receiveAddress());
  }
  ...
  // Keys
  {
    ...
    for (let i = 0; i < 1000; i++) {
      for (let j = 0; j < 10; j++) jobs.push(wallet.createReceive(i));
    }
    ...
    for (const addr of result) addrs.push(addr.getAddress());
  }
