...
(async () => {
  ...
  async function write() {
    for (const length of distribution) {
      ...
      if (written >= settings.total) break;
    }
  }

  while (written < settings.total) await write();
...
class JSONOutput {
  ...
  result(type, start, elapsed, length) {
    if (this.index > 0) process.stdout.write(",");
...
class BenchOutput {
  ...
  result(type, start, elapsed, length) {
    ...
    if (!this.results[type]) this.results[type] = {};

    if (!this.results[type][length]) this.results[type][length] = [];
    ...
    function format(value) {
      ...
      while (value.length < 15) value = `${value} `;
    ...
    function title(value) {
      ...
      while (value.length < 85) value = ` ${value} `;

      if (value.length > 85) value = value.slice(0, 85);
...
function processArgs(argv, config) {
  ...
  for (const key in config) args[key] = config[key].fallback;
  ...
  for (let i = 2; i < process.argv.length; i++) {
    ...
    if (!match) {
      ...
      if (!config[key]) throw new Error(`Invalid argument: ${arg}.`);
      ...
      if (config[key].parse) value = config[key].parse(value);
