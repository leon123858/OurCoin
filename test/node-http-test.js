...
describe("Node HTTP", function () {
  ...
  const pubkeys = [
    Buffer.from(
      "02a7451395735369f2ecdfc829c0f" + "774e88ef1303dfe5b2f04dbaab30a535dfdd6",
      "hex"
    ),
    Buffer.from(
      "03589ae7c835ce76e23cf8feb32f1a" + "df4a7f2ba0ed2ad70801802b0bcd70e99c1c",
      "hex"
    ),
  ];
  ...
  it("should validate a p2wpkh address", async () => {
    ...
    const json = await nclient.execute("validateaddress", [address]);
  ...
  it("should fetch block header by height", async () => {
    ...
    for (const property of properties) assert(property in header);
  ...
  it("should initiate rescan from socket WITH a bloom filter", async () => {
    ...
    nclient.hook("block rescan", (entry, txs) => {
      ...
      matchingBlocks.push(ChainEntry.fromRaw(entry).rhash().toString("hex"));
    });
