...
describe("Wallet RPC Methods", function () {
  ...
  it("should rpc sendmany", async () => {
    ...
    const txid = await wclient.execute("sendmany", [
      "default",
      sendTo,
      1,
      "",
  ...
  it("should fail malformed rpc sendmany", async () => {
    ...
    await assert.rejects(
      async () => {
        await wclient.execute("sendmany", [
          "default",
          sendTo,
          1,
          "",
  ...
  it("should rpc getreceivedbyaddress", async () => {
    ...
    const result1 = await wclient.execute("getreceivedbyaddress", [
      addressOther,
    ]);
    ...
    const result2 = await wclient.execute("getreceivedbyaddress", [
      addressOther,
    ]);
  ...
  it("should not abort rescan if WalletDB is not rescanning.", async () => {
    await assert.rejects(wclient.execute("abortrescan"), {
  ...
  it('should not "rollback to the future"', async () => {
    await assert.rejects(wclient.execute("rescan", [60]), {
  ...
  describe("signmessage", function () {
    ...
    it("should fail with non-wallet address.", async () => {
      await assert.rejects(
        async () => {
          await wclient.execute("signmessage", [nonWalletAddress, message]);
        },
        {
          type: "RPCError",
  ...
  describe("getaddressinfo", () => {
    ...
    const pubkeys = [
      Buffer.from(
        "02a7451395735369f2ecdfc829c0f" +
          "774e88ef1303dfe5b2f04dbaab30a535dfdd6",
        "hex"
      ),
      Buffer.from(
        "03589ae7c835ce76e23cf8feb32f1a" +
          "df4a7f2ba0ed2ad70801802b0bcd70e99c1c",
        "hex"
      ),
    ];
  ...
  describe("importprunedfunds", function () {
    it("will remove and import a tx into the wallet", async () => {
      ...
      const txoutproof = await nclient.execute("gettxoutproof", [
        [txid],
        blocks[0],
      ]);
      assert(txoutproof);
  ...
  describe("contract", () => {
    ...
    it("should rpc transfercontract", async () => {
      await wclient.execute("selectwallet", ["miner"]);
      const tmp = await wclient.execute("transfercontract", [
        uid,
        "new-rpc-test-public-address",
      ]);
      assert.strictEqual(tmp.length, 64);
      const addressOther = await wclient.execute("getnewaddress", []);
      await nclient.execute("generatetoaddress", [1, addressOther]);
    });
  });
