...
function parseTXTest(data) {
  ...
  for (const [txid, index, str, amount] of coins) {
    ...
    if (index >>> 0 === 0xffffffff) continue;
  ...
  return {
    ...
    flags: flags,
    view: view,
    comments: coin ? inspect(coin.script) : "coinbase",
    data: data,
  };
}
...
describe("TX", function () {
  ...
  it("should count sigops for multisig", () => {
    ...
    const input = new Script([Opcode.fromInt(0), Opcode.fromInt(0)]);
    ...
    assert.strictEqual(
      ctx.fund.getSigopsCost(ctx.view, flags),
  ...
  it("should count sigops for p2sh multisig", () => {
    ...
    assert.strictEqual(
      ctx.spend.getSigopsCost(ctx.view, flags),
  ...
  it("should count sigops for nested p2wpkh", () => {
    ...
    const input = new Script([Opcode.fromData(redeem.toRaw())]);
