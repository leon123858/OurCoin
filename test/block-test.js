...
describe("Block", function () {
  ...
  it("should serialize merkle block with transactions", () => {
    ...
    for (const tx of full.txs) {
      if (tree.map.has(tx.hash())) block.txs.push(TX.fromRaw(tx.toRaw()));
    }
  ...
  it("should create a merkle block", () => {
    ...
    const item2 = Buffer.from(
      "047b00000078da0dca3b0ec2300c00d0ab4466ed10" +
  ...
  it("should handle compact block", () => {
    ...
    for (const tx of cblock1.available) assert(tx);
  ...
  for (const cache of [false, true]) {
    ...
    for (const [name, sigops, weight] of sigopsVectors) {
      ...
      it(`should count sigops for ${name} (${word} cache)`, () => {
        ...
        if (!cache) block.refresh(true);

        let count = 0;
        for (const tx of block.txs) count += tx.getSigopsCost(view, flags);
  ...
  it("should deserialize with offset positions for txs (witness)", () => {
    ...
    assert.equal(
      block.getSize(),
      expected.reduce((a, b) => a + b.size, 81)
    );
