...
describe("RPC", function () {
  ...
  describe("Networking", function () {
    ...
    after(async () => {
      if (peer.opened) await peer.close();
    });
  ...
  describe("getblock", function () {
    it("should rpc getblock", async () => {
      const info = await nclient.execute("getblock", [
        util.revHex(node.chain.tip.hash),
      ]);
      const properties = [
        "hash",
        "confirmations",
      ...
      for (const property of properties) assert(property in info);

      assert.strictEqual(node.chain.tip.bits, parseInt(info.bits, 16));
      assert.strictEqual(
        util.revHex(node.chain.tip.merkleRoot),
        info.merkleroot
      );
    ...
    it("should rpc getblockbyheight", async () => {
      ...
      // verbose=0 details=0
      let blockInfo;
      blockInfo = await nclient.execute("getblockbyheight", [
        blockheight,
        0,
        0,
      ]);
      ...
      for (const property of properties) assert(property in blockInfo);
  ...
  describe("signmessagewithprivkey", function () {
    ...
    const expectedSignature =
      "INbVnW4e6PeRmsv2Qgu8NuopvrVjkcxob+sX8OcZG0SALh" +
    ...
    it("should fail on invalid privkey", async () => {
      ...
      await assert.rejects(
        async () => {
          await nclient.execute("signmessagewithprivkey", [privKey, message]);
        },
        {
          type: "RPCError",
          message: "Invalid key.",
  ...
  describe("verifymessage", function () {
    ...
    it("should fail on invalid signature", async () => {
      await assert.rejects(
        async () => {
          await nclient.execute("verifymessage", [address, ".", message]);
        },
        {
          type: "RPCError",
