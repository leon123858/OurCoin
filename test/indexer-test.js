...
describe("Indexer", function () {
  ...
  describe("Index 10 blocks", function () {
    ...
    it("should get txs by address (reverse)", async () => {
      const hashes = await addrindexer.getHashesByAddress(addr, {
        reverse: false,
      });

      assert.strictEqual(hashes.length, 10);

      const reversed = await addrindexer.getHashesByAddress(addr, {
        reverse: true,
      });

      assert.strictEqual(reversed.length, 10);

      for (let i = 0; i < 10; i++) assert.deepEqual(hashes[i], reversed[9 - i]);
    });
    ...
    it("should get txs by address after txid", async () => {
      ...
      const next = await addrindexer.getHashesByAddress(addr, {
        after: txid,
        limit: 5,
      });
    ...
    it("should get txs by address after txid (reverse)", async () => {
      ...
      const all = await addrindexer.getHashesByAddress(addr, { reverse: true });
    ...
    describe("Integration", function () {
      ...
      it("will enable indexes retroactively", async () => {
        ...
        try {
          ...
          const blocks = await nclient.execute("generatetoaddress", [
            150,
            vectors[0].addr,
          ]);
          ...
          if (nclient) await nclient.close();
      ...
      it("will sync if disabled during reorganization", async () => {
        ...
        try {
          ...
          const coinbase = await wclient.execute("getnewaddress", ["default"]);
          ...
          for (let i = 0; i < 10; i++) {
            for (const v of vectors) {
              await wclient.execute("sendtoaddress", [
                v.addr,
                v.amount,
                "",
                "",
          ...
          if (wclient) await wclient.close();
  ...
  describe("HTTP", function () {
    ...
    before(async () => {
      ...
      for (let i = 0; i < 10; i++) {
        for (const v of vectors) {
          const txid = await wclient.execute("sendtoaddress", [
            v.addr,
            v.amount,
            "",
            "",
    ...
    for (const v of vectors) {
      it(`txs by ${v.label} addr`, async () => {
        const res = await nclient.request("GET", `/tx/address/${v.addr}`, {});

        assert.equal(res.length, 15);

        for (let i = 0; i < 10; i++) assert(confirmed.includes(res[i].hash));

        for (let i = 10; i < 15; i++) assert(unconfirmed.includes(res[i].hash));
      });
      ...
      it(`txs by ${v.label} addr (limit)`, async () => {
        ...
        for (const tx of res) assert(confirmed.includes(tx.hash));
      });
      ...
      it(`txs by ${v.label} addr (limit w/ unconf)`, async () => {
        ...
        for (let i = 10; i < 11; i++) assert(unconfirmed.includes(res[i].hash));
      });

      it(`txs by ${v.label} addr (reverse)`, async () => {
        const asc = await nclient.request("GET", `/tx/address/${v.addr}`, {
          reverse: false,
        });

        assert.equal(asc.length, 15);

        const dsc = await nclient.request("GET", `/tx/address/${v.addr}`, {
          reverse: true,
        });

        assert.equal(dsc.length, 15);

        for (let i = 0; i < 10; i++) assert(confirmed.includes(asc[i].hash));

        for (let i = 10; i < 15; i++) assert(unconfirmed.includes(asc[i].hash));
      ...
      it(`txs by ${v.label} addr (after)`, async () => {
        const one = await nclient.request("GET", `/tx/address/${v.addr}`, {
          limit: 3,
        });
        assert.strictEqual(one.length, 3);

        for (let i = 0; i < 3; i++) assert(confirmed.includes(one[i].hash));

        // The after hash is within the
        // confirmed transactions.
        const hash = one[2].hash;

        const two = await nclient.request("GET", `/tx/address/${v.addr}`, {
          after: hash,
          limit: 3,
        });
        assert.strictEqual(one.length, 3);

        const all = await nclient.request("GET", `/tx/address/${v.addr}`, {
          limit: 6,
        });
        assert.strictEqual(one.length, 3);
      ...
      it(`txs by ${v.label} addr (after w/ unconf)`, async () => {
        ...
        for (let i = 0; i < 10; i++) assert(confirmed.includes(one[i].hash));

        for (let i = 10; i < 11; i++) assert(unconfirmed.includes(one[i].hash));
      ...
      it(`txs by ${v.label} addr (after w/ unconf 2)`, async () => {
        ...
        for (let i = 10; i < 12; i++) assert(unconfirmed.includes(one[i].hash));
        ...
        for (let i = 0; i < 3; i++) assert(unconfirmed.includes(two[i].hash));
      ...
      it(`txs by ${v.label} addr (after w/ unconf 3)`, async () => {
        ...
        for (let i = 10; i < 13; i++) assert(unconfirmed.includes(one[i].hash));
      ...
      it(`txs by ${v.label} addr (after, reverse)`, async () => {
        ...
        for (let i = 0; i < 5; i++) assert(unconfirmed.includes(one[i].hash));

        for (let i = 5; i < 8; i++) assert(confirmed.includes(one[i].hash));
        ...
        for (let i = 0; i < 3; i++) assert(confirmed.includes(two[i].hash));
        ...
        for (let i = 0; i < 5; i++) assert(unconfirmed.includes(all[i].hash));

        for (let i = 5; i < 11; i++) assert(confirmed.includes(all[i].hash));
      ...
      it(`txs by ${v.label} addr (after, reverse w/ unconf)`, async () => {
        ...
        for (let i = 5; i < 8; i++) assert(confirmed.includes(all[i].hash));
      ...
      it(`txs by ${v.label} addr (after, reverse w/ unconf 2)`, async () => {
        ...
        assert.strictEqual(one.length, 3);
        for (let i = 0; i < 3; i++) assert(unconfirmed.includes(one[i].hash));
        ...
        for (let i = 0; i < 4; i++) assert(unconfirmed.includes(all[i].hash));
    ...
    describe("Errors", function () {
      it("will give error if limit is exceeded", async () => {
        await assert.rejects(
          async () => {
            await nclient.request("GET", `/tx/address/${vectors[0].addr}`, {
              limit: 101,
            });
          },
          {
