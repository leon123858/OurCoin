  ...
  describe("Packets", function () {
    ...
    it("ping", () => {
      const check = (pkt, nonce) => {
        ...
        if (nonce) assert.bufferEqual(pkt.nonce, Buffer.alloc(8, 0x01));
      };
    ...
    it("CFHeaders", () => {
      ...
      let pkt = new packets.CFHeadersPacket(
        ...
        Array(5).fill(Buffer.from("ab".repeat(32), "hex"))
      );
      check(pkt);
    ...
    it("CFCheckpt", () => {
      ...
      let pkt = new packets.CFCheckptPacket(
        0,
        Buffer.from("ab".repeat(32), "hex"),
        Array(4).fill(Buffer.from("aa".repeat(32), "hex"))
      );
      check(pkt);
    ...
    it("block", () => {
      ...
      const check = (pkt, witness, read) => {
        ...
        if (witness) {
          ...
          if (!read) assert.equal(pkt.witness, true);
        } else {
    ...
    describe("handlePacket", function () {
      ...
      it("will handle types correctly", async () => {
        ...
        for (const [type, handler] of map) {
          ...
          if (handler) assert.equal(stub.count, 1);
          else assert.equal(stub.count, 0);
        }
      });
    });
    ...
    describe("handleGetData", function () {
      ...
      it("will send tx packets", async () => {
        ...
        pool.getItem = (_peer, _item) => {
          ...
          if (_item.hash.compare(tx.hash()) === 0) return tx;
      ...
      it("will send filtered block and txs", async () => {
        ...
        pool.getItem = (_peer, _item) => {
          ...
          if (_item.hash.compare(block.hash()) === 0) return block;
