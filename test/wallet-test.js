...
const KEY1 =
  "xprv9s21ZrQH143K3Aj6xQBymM31Zb4BVc7wxqfUhMZrzewdDVCt" +
  "qUP9iWfcHgJofs25xbaUpCps9GDXj83NiWvQCAkWQhVj5J4CorfnpKX94AZ";

const KEY2 =
  "xprv9s21ZrQH143K3mqiSThzPtWAabQ22Pjp3uSNnZ53A5bQ4udp" +
  "faKekc2m4AChLYH1XDzANhrSdxHYWUeTWjYJwFwWFyHkTMnMeAcW4JyRCZa";

// abandon abandon... about key at m'/44'/0'/0'
const PUBKEY =
  "xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhaw" +
...
describe("Wallet", function () {
  ...
  it("should throw funding error", async () => {
    ...
    await assert.rejects(alice.fund(m2, { useSelectEstimate: true }), {
      type: "FundingError",
    });

    await assert.rejects(alice.fund(m2, { useSelectEstimate: false }), {
      type: "FundingError",
    });
  });
  ...
  for (const type of keyTypes) {
    ...
    it(`should create ${type.name} addresses and get their keys`, async () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        assert(
          key2,
          `Could not get key for ${address.toString()}` +
  ...
  describe("Node Integration", function () {
    ...
    it("should not stack in-memory block queue (oom)", async () => {
      ...
      for (let i = 0; i < 10; i++) await raceForward();
    });
    ...
    it("should emit details with correct confirmation", async () => {
      ...
      wallet.on("tx", (details) => {
        if (details.confirmations === 1) txConfirmedCount += 1;
        else if (details.confirmations === 0) txCount += 1;
      });
      ...
      for (let i = 0; i < 101; i++) await mineBlock();

      await wallet.send({
        outputs: [{ address: waddr, value: 1 * 1e8 }],
        useSelectEstimate: true,
      });
      await mineBlock();
