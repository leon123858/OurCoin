  ...
  toTestFilter(view) {
    ...
    for (let i = 0; i < this.txs.length; i++) {
      ...
      for (const output of tx.outputs) {
        ...
        if (output.script.raw[0] === opcodes.OP_RETURN) continue;
    ...
    for (const [, coins] of view.map) {
      for (const [, coin] of coins.outputs) {
        if (coin.output.script.length === 0) continue;
...
describe("Filter", function () {
  ...
  it("should compute correct filter hash", () => {
    ...
    const filter = new BasicFilter().fromNBytes(Buffer.from("0189a630", "hex"));
