...
describe("P2P", function () {
  ...
  node1.pool.on("packet", (packet) => {
    if (!nodePackets[packet.cmd]) nodePackets[packet.cmd] = [packet];
    else nodePackets[packet.cmd].push(packet);
  });
  ...
  describe("BIP157", function () {
    ...
    it("CFilters", async () => {
      ...
      const pkt = new packets.GetCFiltersPacket(0, 0, node1.chain.tip.hash);
    ...
    it("CFHeaders", async () => {
      ...
      const pkt = new packets.GetCFHeadersPacket(0, 0, node1.chain.tip.hash);
    ...
    it("CFCheckpt", async () => {
      ...
      const pkt = new packets.GetCFCheckptPacket(0, node1.chain.tip.hash);
