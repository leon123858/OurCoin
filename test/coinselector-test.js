...
describe("Coin Selector", function () {
  ...
  const targetSet1 = [
    221000, 220000, 215000, 214000, 211000, 208000, 206000, 203000, 201000,
    195000, 186000, 178000, 166000, 160000, 155000, 152000, 146000, 139000,
    119000, 116000, 110000, 109000, 108000, 106000, 105000, 101000, 98000,
    96000, 90000, 85000, 82000, 81000, 80000, 78000, 71000, 67000, 66000, 63000,
    55000, 53000, 51000, 45000, 44000, 41000, 38000, 36000, 23000, 19000, 16000,
    11000, 6000,
  ];
  ...
  const targetSet3 = [
    219000, 217000, 213000, 212000, 211000, 205000, 202000, 201000, 190000,
    185000, 183000, 182000, 181000, 170000, 155000, 153000, 152000, 151000,
    130000, 120000, 110000, 105000, 103000, 102000, 101000,
  ];
  ...
  describe("Branch and Bound Selection", function () {
    ...
    for (const target of targetSet1) {
      it(`should select target=${
        target - costOfChange
      } using Branch and Bound`, () => {
        const selection = selector.selectBnB(
          target - costOfChange,
          costOfChange
        );
    ...
    for (const target of targetSet1) {
      it(`should fail to select target=${
        target - 500
      } using Branch and Bound`, () => {
...
describe("Integration", function () {
  if (!process.env.BCOIN_RUN_LONG_TESTS) this.skip();
  ...
  for (const useSelectEstimate of [true, false]) {
    it(`should send transactions using ${
      useSelectEstimate ? "old" : "new"
    } selection`, async () => {
      ...
      for (const payment of data) {
        ...
        // remove dust outputs
        if (Math.abs(value) < 500) continue;
